...
**Note:** this repository (`umoria-jp`) is a Japanese localization fork and is not the upstream `dungeons-of-moria/umoria` project. Most of this document (coding style, general contribution etiquette) still applies here, but issue/PR traffic is split between the two repositories — see "Reporting Bugs" below for where to send what.
...
**Where to file it**: translation mistakes and Japanese-display bugs (garbled text, misaligned columns, cut-off labels) belong in this fork's [issues](https://github.com/whitehara/umoria-jp/issues). Gameplay or engine bugs that also reproduce in the English original belong in [upstream's issues](https://github.com/dungeons-of-moria/umoria/issues) instead — filing them here just delays a fix, since this fork doesn't otherwise diverge from upstream's game logic.

Before creating bug reports, please check out the current issues (linked above) as you might find out that you don't need to create one. When you are creating a bug report, please include as many details as possible, as indicated the next section.
...
Bugs are tracked as GitHub issues, in whichever of the two repositories fits per the routing note above. Create an issue explaining the problem, and including any additional details that you think may help maintainers reproduce the problem:
...
Improvements or corrections to the Japanese catalog (`data/lang/ja/umoria.po`)
are welcome. See [`docs/TRANSLATING.md`](docs/TRANSLATING.md) for the catalog
format, the language-selection mechanism, and the display-width helpers
that translated UI text needs to use to avoid layout bugs.

**Do not copy wording from Hengband or other GPL-incompatible projects.**
This fork previously had to review and replace a batch of vocabulary that
had been borrowed from [Hengband](https://hengband.github.io/) (a
non-commercial-only licensed project) — see the "Japanese translation
data and Hengband attribution" section of `README.md` for the background.
New contributions should be original wording, not copied from another
Moria/Angband variant's translation.
