...
void look() {
    if (py.flags.blind > 0) {
        printMessage(_("You can't see a damn thing!"));
        return;
    }

    if (py.flags.image > 0) {
        printMessage(_("You can't believe what you are seeing! It's like a dream!"));
        return;
    }

    int dir;
    if (!getAllDirections(_("Look which direction?"), dir)) {
        return;
    }
    ...
    if (abort) {
        printMessage(_("--Aborting look--"));
        return;
    }

    if (los_num_places_seen != 0) {
        if (dir == 5) {
            printMessage(_("That's all you see."));
        } else {
            printMessage(_("That's all you see in that direction."));
        }
    } else if (dir == 5) {
        printMessage(_("You see nothing of interest."));
    } else {
        printMessage(_("You see nothing of interest in that direction."));
    }
}
...
static bool lookSee(Coord_t coord, bool &transparent) {
    if (coord.x < 0 || coord.y < 0 || coord.y > coord.x) {
        ...
        (void) snprintf(error_message, MORIA_OBJ_DESC_SIZE, _("Illegal call to lookSee(%d, %d)"), coord.y, coord.x);
        printMessage(error_message);
    }

    const char *description = nullptr;
    if (coord.x == 0 && coord.y == 0) {
        description = _("You are on");
    } else {
        description = _("You see");
    }
    ...
    if (los_rocks_and_objects == 0 && tile.creature_id > 1 && monsters[tile.creature_id].lit) {
        ...
        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("%s %s %s. [(r)ecall]"), description, isVowel(creatures_list[j].name[0]) ? _("an") : _("a"), _(creatures_list[j].name));
        description = _("It is on");
    ...
    if (tile.temporary_light || tile.permanent_light || tile.field_mark) {
        ...
        if (tile.treasure_id != 0) {
            ...
            if (los_rocks_and_objects == 0 && game.treasure.list[tile.treasure_id].category_id != TV_INVIS_TRAP) {
                ...
                (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("%s %s ---pause---"), description, obj_string);
                description = _("It is in");
                putStringClearToEOL(msg, Coord_t{0, 0});
        ...
        if (((los_rocks_and_objects != 0) || (msg[0] != 0)) && tile.feature_id >= MIN_CLOSED_SPACE) {
            switch (tile.feature_id) {
                ...
                granite:
                    ...
                    if (msg[0] != 0) {
                        wall_description = _("a granite wall");
                    } else {
                        wall_description = CNIL; // In case we jump here
                    }
                    break;
                case TILE_MAGMA_WALL:
                    wall_description = _("some dark rock");
                    break;
                case TILE_QUARTZ_WALL:
                    wall_description = _("a quartz vein");
                    break;
                default:
            ...
            if (wall_description != nullptr) {
                (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("%s %s ---pause---"), description, wall_description);
                putStringClearToEOL(msg, Coord_t{0, 0});
                panelMoveCursor(coord);
                key = getKeyInput();
            }
