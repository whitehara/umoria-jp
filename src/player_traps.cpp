...
static void playerDisarmFloorTrap(Coord_t coord, int total, int level, int dir, int16_t misc_use) {
    ...
    if (total + 100 - level > randomNumber(100)) {
        printMessage(_("You have disarmed the trap."));
    ...
    if (total > 5 && randomNumber(total) > 5) {
        printMessageNoCommandInterrupt(_("You failed to disarm the trap."));
        return;
    }

    printMessage(_("You set the trap off!"));
...
static void playerDisarmChestTrap(Coord_t coord, int total, Inventory_t &item) {
    if (!spellItemIdentified(item)) {
        ...
        printMessage(_("I don't see a trap."));
    ...
    if ((item.flags & config::treasure::chests::CH_TRAPPED) != 0u) {
        ...
        if ((total - level) > randomNumber(100)) {
            ...
            printMessageNoCommandInterrupt(_("You failed to disarm the chest."));
        } else {
            printMessage(_("You set a trap off!"));
            spellItemIdentifyAndRemoveRandomInscription(item);
            chestTrap(coord);
        }
        return;
    ...
    printMessage(_("The chest was not trapped."));
    game.player_free_turn = true;
}
...
void playerDisarmTrap() {
    ...
    if (no_disarm) {
        printMessage(_("I do not see anything to disarm there."));
        game.player_free_turn = true;
    }
}

static void chestLooseStrength() {
    printMessage(_("A small needle has pricked you!"));
    ...
    playerTakesHit(diceRoll(Dice_t{1, 4}), _("a poison needle"));

    printMessage(_("You feel weakened!"));
}

static void chestPoison() {
    printMessage(_("A small needle has pricked you!"));

    playerTakesHit(diceRoll(Dice_t{1, 6}), _("a poison needle"));

    py.flags.poisoned += 10 + randomNumber(20);
}

static void chestParalysed() {
    printMessage(_("A puff of yellow gas surrounds you!"));
    ...
    printMessage(_("You choke and pass out."));
...
static void chestExplode(Coord_t coord) {
    printMessage(_("There is a sudden explosion!"));

    (void) dungeonDeleteObject(coord);

    playerTakesHit(diceRoll(Dice_t{5, 8}), _("an exploding chest"));
}
