...
static bool playerCanTunnel(int treasure_id, int tile_id) {
    if (tile_id < MIN_CAVE_WALL &&
        ...
        if (treasure_id == 0) {
            printMessage(_("Tunnel through what?  Empty air?!?"));
        } else {
            printMessage(_("You can't tunnel through that."));
        }
...
static void dungeonDigGraniteWall(Coord_t coord, int digging_ability) {
    ...
    if (playerTunnelWall(coord, digging_ability, i)) {
        printMessage(_("You have finished the tunnel."));
    } else {
        printMessageNoCommandInterrupt(_("You tunnel into the granite wall."));
    }
}
...
static void dungeonDigMagmaWall(Coord_t coord, int digging_ability) {
    ...
    if (playerTunnelWall(coord, digging_ability, i)) {
        ...
        printMessageNoCommandInterrupt(_("You tunnel into the magma intrusion."));
    }
}
...
static void dungeonDigQuartzWall(Coord_t coord, int digging_ability) {
    ...
    if (playerTunnelWall(coord, digging_ability, i)) {
        ...
        printMessageNoCommandInterrupt(_("You tunnel into the quartz vein."));
    }
}
...
static void dungeonDigRubble(Coord_t coord, int digging_ability) {
    if (digging_ability > randomNumber(180)) {
        ...
        printMessage(_("You have removed the rubble."));
        ...
        printMessageNoCommandInterrupt(_("You dig in the rubble."));
    }
}
...
static bool dungeonDigAtLocation(Coord_t coord, uint8_t wall_type, int digging_ability) {
    switch (wall_type) {
        ...
        case TILE_BOUNDARY_WALL:
            printMessage(_("This seems to be permanent rock."));
            break;
        default:
            return false;
    }
...
void playerTunnel(int direction) {
    ...
    printMessage(_("You dig with your hands, making no progress."));
}
