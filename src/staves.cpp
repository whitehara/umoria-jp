...
static bool staffPlayerIsCarrying(int &item_pos_start, int &item_pos_end) {
    ...
    if (!inventoryFindRange(TV_STAFF, TV_NEVER, item_pos_start, item_pos_end)) {
        printMessage(_("You are not carrying any staffs."));
        return false;
    }
...
static bool staffPlayerCanUse(Inventory_t &item) {
    ...
    if (randomNumber(chance) < config::player::PLAYER_USE_DEVICE_DIFFICULTY) {
        printMessage(_("You failed to use the staff properly."));
        return false;
    }

    if (item.misc_use < 1) {
        printMessage(_("The staff has no charges left."));
        ...
        switch ((StaffSpellTypes) (getAndClearFirstBit(flags) + 1)) {
            ...
            case StaffSpellTypes::RemoveCurse:
                if (spellRemoveCurseFromAllWornItems()) {
                    if (py.flags.blind < 1) {
                        printMessage(_("The staff glows blue for a moment.."));
                    }
                    identified = true;
                }
                break;
            ...
            default:
                ...
                printMessage(_("Internal error in staffs()"));
                break;
        }
    }
...
void staffUse() {
    ...
    int item_id;
    if (!inventoryGetInputForItemId(item_id, _("Use which staff?"), item_pos_start, item_pos_end, CNIL, CNIL)) {
        return;
    }
    ...
    while (flags != 0) {
        ...
        switch ((WandSpellTypes) (getAndClearFirstBit(flags) + 1)) {
            case WandSpellTypes::WandLight:
                printMessage(_("A line of blue shimmering light appears."));
            ...
            case WandSpellTypes::LightningBall:
                spellFireBall(coord, direction, 32, MagicSpellFlags::Lightning, _("Lightning Ball"));
                identified = true;
                break;
            case WandSpellTypes::ColdBall:
                spellFireBall(coord, direction, 48, MagicSpellFlags::Frost, _("Cold Ball"));
                identified = true;
                break;
            ...
            case WandSpellTypes::AcidBall:
                spellFireBall(coord, direction, 60, MagicSpellFlags::Acid, _("Acid Ball"));
                identified = true;
                break;
            ...
            default:
                // All cases are handled, so this should never be reached!
                printMessage(_("Internal error in wands()"));
                break;
        }
    }
...
void wandAim() {
    ...
    if (!inventoryFindRange(TV_WAND, TV_NEVER, item_pos_start, item_pos_end)) {
        printMessage(_("You are not carrying any wands."));
        return;
    }

    int item_id;
    if (!inventoryGetInputForItemId(item_id, _("Aim which wand?"), item_pos_start, item_pos_end, CNIL, CNIL)) {
        return;
    }
    ...
    if (randomNumber(chance) < config::player::PLAYER_USE_DEVICE_DIFFICULTY) {
        printMessage(_("You failed to use the wand properly."));
        return;
    }

    if (item.misc_use < 1) {
        printMessage(_("The wand has no charges left."));
        if (!spellItemIdentified(item)) {
            itemAppendToInscription(item, config::identification::ID_EMPTY);
        }
        return;
