...
void playerEat() {
    ...
    if (!inventoryFindRange(TV_FOOD, TV_NEVER, item_pos_start, item_pos_end)) {
        printMessage(_("You are not carrying any food."));
        return;
    }

    int item_id;
    if (!inventoryGetInputForItemId(item_id, _("Eat what?"), item_pos_start, item_pos_end, CNIL, CNIL)) {
        return;
    }
        ...
        switch ((FoodMagicTypes) (getAndClearFirstBit(item_flags) + 1)) {
            ...
            case FoodMagicTypes::RestoreSTR:
                if (playerStatRestore(PlayerAttr::A_STR)) {
                    printMessage(_("You feel your strength returning."));
                    identified = true;
                }
                break;
            ...
            case FoodMagicTypes::RestoreINT:
                if (playerStatRestore(PlayerAttr::A_INT)) {
                    printMessage(_("Your head spins a moment."));
                    identified = true;
                }
                break;
            ...
            case FoodMagicTypes::RestoreDEX:
                if (playerStatRestore(PlayerAttr::A_DEX)) {
                    printMessage(_("You feel more dexterous."));
                    identified = true;
                }
                break;
            case FoodMagicTypes::RestoreCHR:
                if (playerStatRestore(PlayerAttr::A_CHR)) {
                    printMessage(_("Your skin stops itching."));
                    identified = true;
                }
                break;
            ...
            case FoodMagicTypes::PoisonousFood:
                playerTakesHit(randomNumber(18), _("poisonous food."));
                identified = true;
                break;
            ...
            default:
                ...
                printMessage(_("Internal error in playerEat()"));
                break;
        }
    }
...
void playerIngestFood(int amount) {
    ...
    if (py.flags.food > config::player::PLAYER_FOOD_MAX) {
        printMessage(_("You are bloated from overeating."));
