...
static void displayCharacterRaces() {
    clearToBottom(20);
    putString(_("Choose a race (? for Help):"), Coord_t{20, 2});
    ...
    for (auto i = 0; i < PLAYER_MAX_RACES; i++) {
        ...
        (void) snprintf(description, MORIA_MESSAGE_SIZE, _("%c) %s"), i + 'a', _(character_races[i].name));

        // Wrap *before* drawing (based on this entry's actual display
        // width, not a fixed byte-based literal) when it would run past
        // the screen edge, so a translated (e.g. Japanese) race name wider
        // than the English original is never clipped mid-entry -- and the
        // column step after drawing is sized the same way, so it never
        // overlaps the next entry either.
        int width = displayWidth(description);
        if (coord.x > 2 && coord.x + width > 78) {
            coord.x = 2;
            coord.y++;
        }
...
static void characterChooseRace() {
    ...
    int id;
    while (true) {
        moveCursor(Coord_t{20, 2 + displayWidth(_("Choose a race (? for Help):")) + 1});
    ...
    // Same fixed CHARACTER_SHEET_LABEL_WIDTH-based column that
    // printCharacterInformation() (src/ui.cpp) clamps the "Race        :"
    // label to, so a translated (e.g. Japanese) label wider than the
    // English original is truncated to fit rather than pushing this value
    // (and the fixed-column Age/Height/Weight block to its right) out of
    // alignment.
    putString(_(character_races[id].name), Coord_t{3, CHARACTER_SHEET_LABEL_WIDTH + 2});
}

// Will print the history of a character -JWT-
static void displayCharacterHistory() {
    putString(_("Character Background"), Coord_t{14, 27});
    ...
    do {
        ...
        while (!flag) {
            if (character_backgrounds[background_id].chart == history_id) {
                ...
                (void) strcat(history_block, _(background.info));
...
static void characterSetGender() {
    clearToBottom(20);
    putString(_("Choose a sex (? for Help):"), Coord_t{20, 2});

    // The left/right choices are separate msgids, joined with the right
    // one padded to a fixed display-column start (not baked into one
    // literal with hardcoded spaces), so a translated (e.g. Japanese)
    // left choice of a different width than the English original never
    // pushes the right choice out of alignment.
    putString((padToDisplayWidth(_("m) Male"), 14) + _("f) Female")).c_str(), Coord_t{21, 2});

    while (true) {
        moveCursor(Coord_t{20, 2 + displayWidth(_("Choose a sex (? for Help):")) + 1});
        const char key = getKeyInput();

        // Same fixed CHARACTER_SHEET_LABEL_WIDTH-based column as
        // characterChooseRace() above -- see there for the rationale.
        const int value_column = CHARACTER_SHEET_LABEL_WIDTH + 2;

        if (key == 'f' || key == 'F') {
            playerSetGender(false);
            putString(_("Female"), Coord_t{4, value_column});
            break;
        } else if (key == 'm' || key == 'M') {
            playerSetGender(true);
            putString(_("Male"), Coord_t{4, value_column});
            break;
        } else if (key == '?') {
            displayTextHelpFile(config::files::welcome_screen);
        } else {
...
static int displayRaceClasses(uint8_t const race_id, uint8_t *class_list) {
    ...
    clearToBottom(20);
    putString(_("Choose a class (? for Help):"), Coord_t{20, 2});

    for (uint8_t i = 0; i < PLAYER_MAX_CLASSES; i++) {
        if ((character_races[race_id].classes_bit_field & mask) != 0u) {
            (void) snprintf(description, MORIA_MESSAGE_SIZE, _("%c) %s"), class_id + 'a', _(classes[i].title));

            // Wrap *before* drawing (based on this entry's actual display
            // width, not a fixed byte-based literal) when it would run
            // past the screen edge, so a translated (e.g. Japanese) class
            // title wider than the English original is never clipped
            // mid-entry -- and the column step after drawing is sized the
            // same way, so it never overlaps the next entry either.
            int width = displayWidth(description);
            if (coord.x > 2 && coord.x + width > 78) {
                coord.x = 2;
                coord.y++;
...
static void generateCharacterClass(uint8_t const class_id) {
    ...
    // Same fixed CHARACTER_SHEET_LABEL_WIDTH-based column as
    // characterChooseRace() above -- see there for the rationale.
    putString(_(klass.title), Coord_t{5, CHARACTER_SHEET_LABEL_WIDTH + 2});
...
static void characterGetClass() {
    ...
    while (true) {
        moveCursor(Coord_t{20, 2 + displayWidth(_("Choose a class (? for Help):")) + 1});
        const char key = getKeyInput();
...
void characterCreate() {
    ...
    while (!done) {
        ...
        clearToBottom(20);
        putString(_("Hit space to re-roll or ESC to accept characteristics: "), Coord_t{20, 2});
    ...
    putStringClearToEOL(_("[ press any key to continue, or Q to exit ]"), Coord_t{23, 17});
    if (getKeyInput() == 'Q') {
        exitProgram();
    }
