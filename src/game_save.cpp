...
bool saveGame() {
    ...
    while (!saveChar(config::files::save_game)) {
        output = _("Save file '") + config::files::save_game + _("' fails.");
        printMessage(output.c_str());

        int i = 0;
        if (access(config::files::save_game.c_str(), 0) < 0 || !getInputConfirmation(_("File exists. Delete old save file?")) ||
            (i = unlink(config::files::save_game.c_str())) < 0) {
            if (i < 0) {
                output = _("Can't delete '") + config::files::save_game + "'";
                printMessage(output.c_str());
            }
            const char *save_file_prompt = _("New Save file [ESC to give up]:");
            putStringClearToEOL(save_file_prompt, Coord_t{0, 0});
            if (!getStringInput(input, Coord_t{0, displayWidth(save_file_prompt)}, 45)) {
                return false;
            }
            if (input[0] != 0) {
        ...
        output = _("Saving with '") + config::files::save_game + "'...";
...
static bool saveChar(const std::string &filename) {
    ...
    if (fd < 0 && access(filename.c_str(), 0) >= 0 && ((from_save_file != 0) || (game.wizard_mode && getInputConfirmation(_("Can't make new save file. Overwrite old?"))))) {
    ...
    if (!ok) {
        ...
        std::string output;
        if (fd >= 0) {
            output = _("Error writing to file '") + filename + "'";
        } else {
            output = _("Can't create new file '") + filename + "'";
        }
        printMessage(output.c_str());
...
bool loadGame(bool &generate) {
    ...
    if (access(config::files::save_game.c_str(), 0) != 0) {
        printMessage(_("Save file does not exist."));
        return false; // Don't bother with messages here. File absent.
    }

    clearScreen();

    std::string filename = _("Save file '") + config::files::save_game + _("' present. Attempting restore.");
    putString(filename.c_str(), Coord_t{23, 0});

    // FIXME: check this if/else logic! -- MRC
    if (dg.game_turn >= 0) {
        printMessage(_("IMPOSSIBLE! Attempt to restore while still alive!"));
        ...
        printMessage(_("Can't open file for reading."));
    } else {
        dg.game_turn = -1;
        bool ok = true;
        ...
        putStringClearToEOL(_("Restoring Memory..."), Coord_t{0, 0});
        putQIO();
        ...
        if (!validGameVersion(version_maj, version_min, patch_level)) {
            putStringClearToEOL(_("Sorry. This save file is from a different version of umoria."), Coord_t{2, 0});
            goto error;
        }
        ...
        if (game.to_be_wizard && ((l & 0x40000000L) != 0)) {
            printMessage(_("Sorry, this character is retired from moria."));
            printMessage(_("You can not resurrect a retired character."));
        } else if (game.to_be_wizard && ((l & 0x80000000L) != 0) && getInputConfirmation(_("Resurrect a dead character?"))) {
            l &= ~0x80000000L;
        }
        ...
        if (c == EOF || ((l & 0x80000000L) != 0)) {
            if ((l & 0x80000000L) == 0) {
                ...
                putStringClearToEOL(_("Attempting a resurrection!"), Coord_t{0, 0});
                ...
                printMessage(_("Restoring Memory of a departed spirit..."));
                dg.game_turn = -1;
            }
            putQIO();
            goto closefiles;
        ...
        putStringClearToEOL(_("Restoring Character..."), Coord_t{0, 0});
        putQIO();
    ...
    closefiles:
        ...
        if (!ok) {
            printMessage(_("Error during reading of file."));
        } else {
            // let the user overwrite the old save file when save/quit
            from_save_file = 1;

            if (panic_save) {
                printMessage(_("This game is from a panic save.  Score will not be added to scoreboard."));
            } else if ((!game.noscore) & 0x04) {
                printMessage(_("This character is already on the scoreboard; it will not be scored again."));
                game.noscore |= 0x4;
            }
    ...
    dg.game_turn = -1;
    putStringClearToEOL(_("Please try again without that save file."), Coord_t{1, 0});
