...
static bool canReadSpells() {
    ...
    if (classes[py.misc.class_id].class_to_use_mage_spells != config::spells::SPELL_TYPE_MAGE) {
        printMessage(_("You can't cast spells!"));
        return false;
    }
...
void getAndCastMagicSpell() {
    ...
    if (!inventoryFindRange(TV_MAGIC_BOOK, TV_NEVER, i, j)) {
        printMessage(_("But you are not carrying any spell-books!"));
        return;
    }

    int item_val;
    if (!inventoryGetInputForItemId(item_val, _("Use which spell-book?"), i, j, CNIL, CNIL)) {
        return;
    }

    int choice, chance;
    int result = castSpellGetId(_("Cast which spell?"), item_val, choice, chance);
    if (result < 0) {
        printMessage(_("You don't know any spells in that book."));
        return;
    }
    if (result == 0) {
        return;
    ...
    if (randomNumber(100) < chance) {
        printMessage(_("You failed to get the spell off!"));
    } else {
        castSpell(choice + 1);
    ...
    if (magic_spell.mana_required > py.misc.current_mana) {
        printMessage(_("You faint from the effort!"));
