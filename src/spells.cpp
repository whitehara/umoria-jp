...
static bool spellGetId(int *spell_ids, int number_of_choices, int &spell_id, int &spell_chance, const char *prompt, int first_spell) {
    ...
    vtype_t str = {'\0'};
    (void) snprintf(str, MORIA_MESSAGE_SIZE, _("(Spells %c-%c, *=List, <ESCAPE>=exit) %s"), //
    ...
    while (!spell_found && getMenuItemId(str, spellChoice)) {
        if (isupper((int) spellChoice) != 0) {
            ...
            if (test_spell_id == number_of_choices) {
                ...
                vtype_t tmp_str = {'\0'};
                (void) snprintf(tmp_str, MORIA_MESSAGE_SIZE, _("Cast %s (%d mana, %d%% fail)?"), _(spell_names[spell_id + offset]), spell.mana_required,
        ...
        if (spell_id == -2) {
            vtype_t tmp_str = {'\0'};
            (void) snprintf(tmp_str, MORIA_MESSAGE_SIZE, _("You don't know that %s."), (offset == config::spells::NAME_OFFSET_SPELLS ? _("spell") : _("prayer")));
...
int castSpellGetId(const char *prompt, int item_id, int &spell_id, int &spell_chance) {
    ...
    if ((result != 0) && magic_spells[py.misc.class_id - 1][spell_id].mana_required > py.misc.current_mana) {
        if (classes[py.misc.class_id].class_to_use_mage_spells == config::spells::SPELL_TYPE_MAGE) {
            result = (int) getInputConfirmation(_("You summon your limited strength to cast this one! Confirm?"));
        } else {
            result = (int) getInputConfirmation(_("The gods may think you presumptuous for this! Confirm?"));
        }
    }
...
bool spellDetectInvisibleCreaturesWithinVicinity() {
    ...
    if (detected) {
        printMessage(_("You sense the presence of invisible creatures!"));
        printMessage(CNIL);
...
bool spellLightArea(Coord_t coord) {
    if (py.flags.blind < 1) {
        printMessage(_("You are surrounded by a white light."));
    }
...
bool spellDarkenArea(Coord_t coord) {
    ...
    if (darkened && py.flags.blind < 1) {
        printMessage(_("Darkness surrounds you."));
    }
...
bool spellIdentifyItem() {
    int item_id;
    if (!inventoryGetInputForItemId(item_id, _("Item you wish identified?"), 0, PLAYER_INVENTORY_SIZE, CNIL, CNIL)) {
        return false;
    }
    ...
    if (item_id >= PlayerEquipment::Wield) {
        ...
        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("%s: %s"), playerItemWearingDescription(item_id), description);
    } else {
        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("%c %s"), item_id + 97, description);
    }
    printMessage(msg);
...
bool spellAggravateMonsters(int affect_distance) {
    ...
    if (aggravated) {
        printMessage(_("You hear a sudden stirring in the distance!"));
    }
    ...
    for (coord.y = py.pos.y - 1; coord.y <= py.pos.y + 1; coord.y++) {
        for (coord.x = py.pos.x - 1; coord.x <= py.pos.x + 1; coord.x++) {
            ...
            if ((item.category_id >= TV_INVIS_TRAP && item.category_id <= TV_CLOSED_DOOR && item.category_id != TV_RUBBLE) || item.category_id == TV_SECRET_DOOR) {
                ...
                printMessage(_("You have disarmed the chest."));
                spellItemIdentifyAndRemoveRandomInscription(item);
            }
        }
    }
...
bool spellDetectMonsters() {
    ...
    if (detected) {
        printMessage(_("You sense the presence of monsters!"));
        printMessage(CNIL);
...
static void spellLightLineTouchesMonster(int monster_id) {
    ...
    if ((creature.defenses & config::monsters::defense::CD_LIGHT) != 0) {
        ...
        if (monsterTakeHit(monster_id, diceRoll(Dice_t{2, 8})) >= 0) {
            printMonsterActionText(name, _("shrivels away in the light!"));
            displayCharacterExperience();
        } else {
            printMonsterActionText(name, _("cringes from the light!"));
        }
    }
}
...
void spellStarlite(Coord_t coord) {
    if (py.flags.blind < 1) {
        printMessage(_("The end of the staff bursts into a blue shimmering light."));
    }
    ...
    switch (spell_type) {
        ...
        case MagicSpellFlags::HolyOrb:
            ...
            *destroy = setNull;
            break;
        default:
            printMessage(_("ERROR in spellGetAreaAffectFlags()\n"));
    }
}
...
static void printBoltStrikesMonsterMessage(Creature_t const &creature, const std::string &bolt_name, bool is_lit) {
    ...
    if (lang::currentLanguage() == "ja") {
        // Japanese has no articles: bare name (or "it") only.
        monster_name = is_lit ? std::string(_(creature.name)) : _("it");
    } else if (is_lit) {
        monster_name = _("the ") + std::string(_(creature.name));
    } else {
        monster_name = _("it");
    }
    std::string msg = lang::format(_("The %s strikes %s."), {_(bolt_name), monster_name});
...
static void spellFireBoltTouchesMonster(Tile_t &tile, int damage, int harm_type, uint32_t weapon_id, const std::string &bolt_name) {
    ...
    if (monsterTakeHit((int) tile.creature_id, damage) >= 0) {
        printMonsterActionText(name, _("dies in a fit of agony."));
        displayCharacterExperience();
    } else if (damage > 0) {
        printMonsterActionText(name, _("screams in agony."));
    }
}
...
void spellFireBall(Coord_t coord, int direction, int damage_hp, int spell_type, const std::string &spell_name) {
    ...
    while (!finished) {
        ...
        if (tile->feature_id >= MIN_CLOSED_SPACE || tile->creature_id > 1) {
            ...
            if (total_hits == 1) {
                printMessage(lang::format(_("The %s envelops a creature!"), {_(spell_name)}).c_str());
            } else if (total_hits > 1) {
                printMessage(lang::format(_("The %s envelops several creatures!"), {_(spell_name)}).c_str());
            }

            if (total_kills == 1) {
                printMessage(_("There is a scream of agony!"));
            } else if (total_kills > 1) {
                printMessage(_("There are several screams of agony!"));
            }
            ...
            if (coordInBounds(location) && coordDistanceBetween(coord, location) <= max_distance && los(coord, location)) {
                ...
                if (tile.feature_id <= MAX_OPEN_SPACE) {
                    ...
                    if (tile.creature_id > 1) {
                        ...
                        switch (spell_type) {
                            case MagicSpellFlags::Lightning:
                                damageLightningBolt(damage, _(spell_name.c_str()));
                                break;
                            case MagicSpellFlags::PoisonGas:
                                damagePoisonedGas(damage, _(spell_name.c_str()));
                                break;
                            case MagicSpellFlags::Acid:
                                damageAcid(damage, _(spell_name.c_str()));
                                break;
                            case MagicSpellFlags::Frost:
                                damageCold(damage, _(spell_name.c_str()));
                                break;
                            case MagicSpellFlags::Fire:
                                damageFire(damage, _(spell_name.c_str()));
                                break;
                            default:
                                break;
                        }
...
bool spellRechargeItem(int number_of_charges) {
    ...
    if (!inventoryFindRange(TV_STAFF, TV_WAND, item_pos_start, item_pos_end)) {
        printMessage(_("You have nothing to recharge."));
        return false;
    }

    int item_id;
    if (!inventoryGetInputForItemId(item_id, _("Recharge which item?"), item_pos_start, item_pos_end, CNIL, CNIL)) {
        return false;
    }
    ...
    if (fail_chance == 1) {
        printMessage(_("There is a bright flash of light."));
    ...
    while (!finished) {
        ...
        if (tile.creature_id > 1) {
            ...
            if (speed > 0) {
                ...
                printMonsterActionText(name, _("starts moving faster."));
                ...
                printMonsterActionText(name, _("starts moving slower."));
            } else {
                monster.sleep_count = 0;
    ...
    while (!finished) {
        ...
        if (tile.creature_id > 1) {
            ...
            if (randomNumber(MON_MAX_LEVELS) < creature.level || ((creature.defenses & config::monsters::defense::CD_NO_SLEEP) != 0)) {
                ...
                printMonsterActionText(name, _("appears confused."));
            }
        }
    }
    ...
    while (!finished) {
        ...
        if (tile.feature_id >= MIN_CAVE_WALL && tile.feature_id != TILE_BOUNDARY_WALL) {
            ...
            if (caveTileVisible(coord)) {
                turned = true;
                printMessage(_("The wall turns into mud."));
            }
            ...
            if (coordInsidePanel(coord) && caveTileVisible(coord)) {
                ...
                (void) snprintf(out_val, MORIA_OBJ_DESC_SIZE, _("The %s turns into mud."), description);
            ...
            if (game.treasure.list[tile.treasure_id].category_id == TV_RUBBLE) {
                ...
                if (randomNumber(10) == 1) {
                    ...
                    if (caveTileVisible(coord)) {
                        printMessage(_("You have found something!"));
                    }
                }
                dungeonLiteSpot(coord);
            } else {
        ...
        if (tile.creature_id > 1) {
            ...
            if ((creature.defenses & config::monsters::defense::CD_STONE) != 0) {
                ...
                if (creature_id >= 0) {
                    ...
                    printMonsterActionText(name, _("dissolves!"));
                    ...
                    printMonsterActionText(name, _("grunts in pain!"));
                }
                finished = true;
            }
        }
    ...
    while (!finished) {
        ...
        if (tile.creature_id > 1) {
            ...
            if ((creature.movement & config::monsters::move::CM_PHASE) == 0u) {
                ...
                printMonsterActionText(name, _("wails out in pain!"));

                if (monsterTakeHit((int) tile.creature_id, damage) >= 0) {
                    printMonsterActionText(name, _("is embedded in the rock."));
                    displayCharacterExperience();
                }
...
bool spellGenocide() {
    char creature_char;
    if (!getTileCharacter(_("Which type of creature do you wish exterminated?"), creature_char)) {
        return false;
    }
    ...
    for (int id = next_free_monster_id - 1; id >= config::monsters::MON_MIN_INDEX_ID; id--) {
        ...
        if (creature_char == creatures_list[monster.creature_id].sprite) {
            if ((creature.movement & config::monsters::move::CM_WIN) == 0) {
                ...
                if (lang::currentLanguage() == "ja") {
                    // Japanese has no articles: bare name only.
                    printMessage(lang::format(_("%s is unaffected."), {std::string(_(creature.name))}).c_str());
                } else {
                    printMessage(lang::format(_("The %s is unaffected."), {std::string(_(creature.name))}).c_str());
                }
            }
        }
    }
...
bool spellDetectEvil() {
    ...
    if (detected) {
        printMessage(_("You sense the presence of evil!"));
        printMessage(CNIL);
...
bool spellChangePlayerHitPoints(int adjustment) {
    ...
    if (adjustment < 3) {
        if (adjustment == 0) {
            printMessage(_("You feel a little better."));
        } else {
            printMessage(_("You feel better."));
        }
    } else {
        if (adjustment < 7) {
            printMessage(_("You feel much better."));
        } else {
            printMessage(_("You feel very good."));
        }
    }
...
void spellCreateFood() {
    ...
    if (tile.treasure_id != 0) {
        ...
        printMessage(_("There is already an object under you."));
    ...
    for (int id = next_free_monster_id - 1; id >= config::monsters::MON_MIN_INDEX_ID; id--) {
        ...
        if (monster.distance_from_player <= config::monsters::MON_MAX_SIGHT && ((creature_defense & creatures_list[monster.creature_id].defenses) != 0) &&
            ...
            if (hit >= 0) {
                ...
                printMonsterActionText(name, _("shudders."));
            }
        ...
        if (monster.distance_from_player <= config::monsters::MON_MAX_SIGHT && ((creature.defenses & config::monsters::defense::CD_UNDEAD) != 0) && los(py.pos, monster.pos)) {
            ...
            if (py.misc.level + 1 > creature.level || randomNumber(5) == 1) {
                if (monster.lit) {
                    ...
                    printMonsterActionText(name, _("runs frantically!"));
                }
...
void spellLoseSTR() {
    if (!py.flags.sustain_str) {
        ...
        printMessage(_("You feel sick for a moment,  it passes."));
    }
}
...
void spellLoseINT() {
    if (!py.flags.sustain_int) {
        ...
        printMessage(_("You become very dizzy."));
    } else {
        printMessage(_("You become dizzy for a moment,  it passes."));
    }
}
...
void spellLoseWIS() {
    if (!py.flags.sustain_wis) {
        ...
        printMessage(_("You feel very naive."));
    } else {
        printMessage(_("You feel naive for a moment,  it passes."));
    }
}
...
void spellLoseDEX() {
    if (!py.flags.sustain_dex) {
        ...
        printMessage(_("You feel very sore."));
    } else {
        printMessage(_("You feel sore for a moment,  it passes."));
    }
}
...
void spellLoseCHR() {
    if (!py.flags.sustain_chr) {
        ...
        printMessage(_("Your skin starts to itch."));
    } else {
        printMessage(_("Your skin starts to itch, but feels better now."));
    }
}
...
bool spellSlowPoison() {
    if (py.flags.poisoned > 0) {
        ...
        printMessage(_("The effect of the poison has been reduced."));
        return true;
    }
...
void spellDestroyArea(Coord_t coord) {
    ...
    printMessage(_("There is a searing blast of light!"));
...
bool spellRestorePlayerLevels() {
    if (py.misc.max_exp > py.misc.exp) {
        printMessage(_("You feel your life energies returning."));
