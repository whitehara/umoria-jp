...
static void printSpeechFinishedHaggling() {
    printMessage(_(speech_sale_accepted[randomNumber(14) - 1]));
}

// %A1 is offer, %A2 is asking.
static void printSpeechSellingHaggle(int32_t offer, int32_t asking, int final) {
    obj_desc_t comment = {'\0'};

    if (final > 0) {
        (void) strcpy(comment, _(speech_selling_haggle_final[randomNumber(3) - 1]));
    } else {
        (void) strcpy(comment, _(speech_selling_haggle[randomNumber(16) - 1]));
    }
...
static void printSpeechBuyingHaggle(int32_t offer, int32_t asking, int final) {
    ...
    if (final > 0) {
        (void) strcpy(comment, _(speech_buying_haggle_final[randomNumber(3) - 1]));
    } else {
        (void) strcpy(comment, _(speech_buying_haggle[randomNumber(15) - 1]));
    }
...
static void printSpeechGetOutOfMyStore() {
    ...
    printMessage(_(speech_insulted_haggling_done[comment]));
    printMessage(_(speech_get_out_of_my_store[comment]));
}

static void printSpeechTryAgain() {
    printMessage(_(speech_haggling_try_again[randomNumber(10) - 1]));
}

static void printSpeechSorry() {
    printMessage(_(speech_sorry[randomNumber(5) - 1]));
}

// Displays the set of commands -RAK-
static void displayStoreCommands() {
    putStringClearToEOL(_("You may:"), Coord_t{20, 0});

    // The left/right columns are separate msgids, joined with the right
    // column padded to a fixed display-column start (not baked into one
    // literal with hardcoded spaces), so a translated (e.g. Japanese)
    // left column of a different width than the English original never
    // pushes the right column out of alignment or off-screen.
    const int right_column = 32;

    putStringClearToEOL(padToDisplayWidth(_(" p) Purchase an item."), right_column) + _("b) Browse store's inventory."), Coord_t{21, 0});
    putStringClearToEOL(padToDisplayWidth(_(" s) Sell an item."), right_column) + _("i/e/t/w/x) Inventory/Equipment Lists."), Coord_t{22, 0});
    putStringClearToEOL(padToDisplayWidth(_("ESC) Exit from Building."), right_column) + _("^R) Redraw the screen."), Coord_t{23, 0});
}

// Displays the set of commands -RAK-
static void displayStoreHaggleCommands(int haggle_type) {
    if (haggle_type == -1) {
        putStringClearToEOL(_("Specify an asking-price in gold pieces."), Coord_t{21, 0});
    } else {
        putStringClearToEOL(_("Specify an offer in gold pieces."), Coord_t{21, 0});
    }

    putStringClearToEOL(_("ESC) Quit Haggling."), Coord_t{22, 0});
...
static void displayStoreInventory(Store_t &store, int item_pos_start) {
    ...
    for (item_line_num = (item_pos_start % 12); item_pos_start < item_pos_end; item_line_num++) {
        ...
        obj_desc_t msg = {'\0'};
        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("%c) %s"), 'a' + item_line_num, description);
        ...
        if (current_item_count <= 0) {
            ...
            (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("%9d [Fixed]"), current_item_count);
        }
    ...
    if (store.unique_items_counter > 12) {
        putString(_("- cont. -"), Coord_t{17, 60});
    } else {
...
static void displaySingleCost(int store_id, int item_id) {
    ...
    if (cost < 0) {
        ...
        (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("%9d [Fixed]"), cost);
    }
...
static void displayPlayerRemainingGold() {
    vtype_t msg = {'\0'};
    (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("Gold Remaining : %d"), py.misc.au);
...
static void displayStore(Store_t &store, const char *owner_name, int current_top_item_id) {
    clearScreen();
    putString(_(owner_name), Coord_t{3, 9});
    putString(_("Item"), Coord_t{4, 3});
    putString(_("Asking Price"), Coord_t{4, 60});
...
static bool storeGetItemId(int &item_id, const char *prompt, int item_pos_start, int item_pos_end) {
    ...
    vtype_t msg = {'\0'};
    (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("(Items %c-%c, ESC to exit) %s"), item_pos_start + 'a', item_pos_end + 'a', prompt);
...
static bool storeGetHaggle(const char *prompt, int32_t &new_offer, int offer_count) {
    ...
    // Cursor/overlay column, not byte count: `prompt` can be translated
    // (Japanese) text, where multi-byte characters make strlen() overcount
    // relative to the columns actually occupied on screen.
    auto prompt_len = displayWidth(prompt);
    ...
    while (valid_offer && adjustment == 0) {
        ...
        if ((offer_count != 0) && store_last_increment != 0) {
            ...
            (void) snprintf(last_offer_str, MORIA_MESSAGE_SIZE, _("[%c%d] "), (store_last_increment < 0) ? '-' : '+', abs_store_last_increment);
            putStringClearToEOL(last_offer_str, Coord_t{0, start_len});

            prompt_len = start_len + displayWidth(last_offer_str);
        }
        ...
        if (valid_offer && offer_count == 0 && increment) {
            printMessage(_("You haven't even made your first offer yet!"));
            adjustment = 0;
            increment = false;
        }
    }
...
static BidState storePurchaseHaggle(int store_id, int32_t &price, Inventory_t const &item) {
    ...
    if (storeNoNeedToBargain(stores[store_id], final_asking_price)) {
        printMessage(_("After a long bargaining session, you agree upon the price."));
        current_asking_price = min_sell;
        comment = _("Final offer");
    ...
    while (!rejected) {
        do {
            ...
            vtype_t msg = {'\0'};
            (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("%s :  %d"), comment, current_asking_price);
            putString(msg, Coord_t{1, 0});

            status = storeReceiveOffer(store_id, _("What do you offer? "), new_offer, last_offer, offers_count, 1);
        ...
        if (!rejected) {
            ...
            if (!rejected) {
                ...
                vtype_t msg = {'\0'};
                (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("Your last offer : %d"), last_offer);
...
static BidState storeSellHaggle(int store_id, int32_t &price, Inventory_t const &item) {
    ...
    if (!rejected) {
        ...
        if (max_buy > max_gold) {
            ...
            printMessage(_("I am sorry, but I have not the money to afford such a fine item."));
            accepted_without_haggle = true;
        } else {
        ...
        do {
            do {
                ...
                status = storeReceiveOffer(store_id, _("What price do you ask? "), new_offer, last_offer, offer_count, -1);
            ...
            if (!rejected) {
                ...
                if (!rejected) {
                    ...
                    eraseLine(Coord_t{1, 0});
                    vtype_t msg = {'\0'};
                    (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("Your last bid %d"), last_offer);
                    putString(msg, Coord_t{1, 39});
...
static bool storePurchaseAnItem(int store_id, int &current_top_item_id) {
    ...
    if (store.unique_items_counter < 1) {
        printMessage(_("I am currently out of stock."));
        return false;
    }

    int item_id;
    int item_count = storeItemsToDisplay(store.unique_items_counter, current_top_item_id);
    if (!storeGetItemId(item_id, _("Which item are you interested in? "), 0, item_count)) {
        return false;
    }
    ...
    if (!inventoryCanCarryItemCount(sell_item)) {
        putStringClearToEOL(_("You cannot carry that many different items."), Coord_t{0, 0});
        return false;
    }
    ...
    if (status == BidState::Insulted) {
        ...
        if (py.misc.au >= price) {
            ...
            obj_desc_t msg = {'\0'};
            (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("You have %s (%c)"), description, new_item_id + 'a');
            ...
            if (storeIncreaseInsults(store_id)) {
                ...
                printSpeechFinishedHaggling();
                printMessage(_("Liar!  You have not the gold!"));
            }
        }
    }
...
static bool storeSellAnItem(int store_id, int &current_top_item_id) {
    ...
    if (last_item == -1) {
        printMessage(_("You have nothing to sell to this store!"));
        return false;
    }

    int item_id;
    if (!inventoryGetInputForItemId(item_id, _("Which one? "), first_item, last_item, mask, _("I do not buy such items."))) {
        return false;
    }
    ...
    obj_desc_t msg = {'\0'};
    (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("Selling %s (%c)"), description, item_id + 'a');
    printMessage(msg);

    if (!storeCheckPlayerItemsCount(stores[store_id], sold_item)) {
        printMessage(_("I have not the room in my store to keep it."));
        return false;
    }
    ...
    if (status == BidState::Insulted) {
        ...
        printMessage(_("How dare you!"));
        printMessage(_("I will not buy that!"));
        ...
        itemDescription(description, sold_item, true);
        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("You've sold %s"), description);
        printMessage(msg);
...
void storeEnter(int store_id) {
    ...
    if (store.turns_left_before_closing >= dg.game_turn) {
        printMessage(_("The doors are locked."));
        return;
    }
    ...
    while (!exit_store) {
        moveCursor(Coord_t{20, displayWidth(_("You may:")) + 1});
                ...
                case 'b':
                    if (current_top_item_id == 0) {
                        if (store.unique_items_counter > 12) {
                            ...
                            printMessage(_("Entire inventory is shown."));
                        }
                    } else {
                        current_top_item_id = 0;
                        displayStoreInventory(stores[store_id], current_top_item_id);
