...
static void inventoryDropOrThrowItem(Coord_t coord, Inventory_t *item) {
    ...
    if (flag) {
        ...
        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("The %s disappears."), description);
        printMessage(msg);
    }
}
...
void playerThrowItem() {
    ...
    int item_id;
    if (!inventoryGetInputForItemId(item_id, _("Fire/Throw which one?"), 0, py.pack.unique_items - 1, CNIL, CNIL)) {
        return;
    }
        ...
        if (tile.feature_id <= MAX_OPEN_SPACE && !flag) {
            if (tile.creature_id > 1) {
                ...
                if (playerTestBeingHit(tbth, (int) py.misc.level, tpth, (int) creatures_list[m_ptr.creature_id].ac, PlayerClassLevelAdj::BTHB)) {
                    ...
                    if (!m_ptr.lit) {
                        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("You hear a cry as the %s finds a mark."), description);
                        visible = false;
                    } else {
                        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("The %s hits the %s."), description, _(creatures_list[damage].name));
                        visible = true;
                    }
                    printMessage(msg);
                    ...
                    if (damage >= 0) {
                        if (!visible) {
                            printMessage(_("You have killed something!"));
                        } else {
                            (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("You have killed the %s."), _(creatures_list[damage].name));
                            printMessage(msg);
                        }
