...
void startMoria(uint32_t seed, bool start_new_game, bool roguelike_keys) {
    ...
    //
    clearScreen();
    putString(_("Press ? for help"), Coord_t{0, 63});
    ...
    while (!game.character_is_dead) {
        ...
        if (eof_flag != 0) {
            (void) strcpy(game.character_died_from, _("(end of input: saved)"));
            if (!saveGame()) {
                (void) strcpy(game.character_died_from, _("unexpected eof"));
            }
    ...
    if (py.carrying_light) {
        if (item.misc_use > 0) {
            ...
            if (item.misc_use == 0) {
                ...
                printMessage(_("Your light has gone out!"));
                playerDisturb(0, 1);
                ...
                playerDisturb(0, 0);
                printMessage(_("Your light is growing faint."));
            }
        } else {
            py.carrying_light = false;
            playerDisturb(0, 1);
...
static void playerActivateHeroism() {
    ...
    printMessage(_("You feel like a HERO!"));
...
static void playerDisableHeroism() {
    ...
    printMessage(_("The heroism wears off."));
    printCharacterMaxHitPoints();
}
...
static void playerActivateSuperHeroism() {
    ...
    printMessage(_("You feel like a SUPER HERO!"));
    printCharacterMaxHitPoints();
    printCharacterCurrentHitPoints();
}
...
static void playerDisableSuperHeroism() {
    ...
    printMessage(_("The super heroism wears off."));
    printCharacterMaxHitPoints();
}
...
static int playerFoodConsumption() {
    ...
    if (py.flags.food < config::player::PLAYER_FOOD_ALERT) {
        if (py.flags.food < config::player::PLAYER_FOOD_WEAK) {
            ...
            if ((py.flags.status & config::player::status::PY_WEAK) == 0) {
                ...
                printMessage(_("You are getting weak from hunger."));
                playerDisturb(0, 0);
    ...
    if (py.flags.food < 0) {
        playerTakesHit(-py.flags.food / 16, _("starvation")); // -CJS-
        playerDisturb(1, 0);
    }
...
static void playerUpdateBlindness() {
    ...
    if (py.flags.blind == 0) {
        ...
        printMessage(_("The veil of darkness lifts."));
    }
}
...
static void playerUpdateConfusion() {
    ...
    if (py.flags.confused == 0) {
        ...
        printCharacterConfusedState();
        printMessage(_("You feel less confused now."));
...
static void playerUpdateFearState() {
    ...
    if (py.flags.afraid == 0) {
        ...
        printCharacterFearState();
        printMessage(_("You feel bolder now."));
        playerDisturb(0, 0);
    }
}
...
static void playerUpdatePoisonedState() {
    ...
    if (py.flags.poisoned == 0) {
        ...
        printCharacterPoisonedState();
        printMessage(_("You feel better."));
        playerDisturb(0, 0);
    ...
    playerTakesHit(damage, _("poison"));
    playerDisturb(1, 0);
}
...
static void playerUpdateFastness() {
    ...
    if ((py.flags.status & config::player::status::PY_FAST) == 0) {
        ...
        printMessage(_("You feel yourself moving faster."));
        playerDisturb(0, 0);
    }
    ...
    if (py.flags.fast == 0) {
        ...
        printMessage(_("You feel yourself slow down."));
        playerDisturb(0, 0);
    }
}
...
static void playerUpdateSlowness() {
    ...
    if ((py.flags.status & config::player::status::PY_SLOW) == 0) {
        ...
        printMessage(_("You feel yourself moving slower."));
        playerDisturb(0, 0);
    }
    ...
    if (py.flags.slow == 0) {
        ...
        printMessage(_("You feel yourself speed up."));
        playerDisturb(0, 0);
    }
}
...
static void playerUpdateEvilProtection() {
    ...
    if (py.flags.protect_evil == 0) {
        printMessage(_("You no longer feel safe from evil."));
    }
}
...
static void playerUpdateInvulnerability() {
    ...
    if ((py.flags.status & config::player::status::PY_INVULN) == 0) {
        ...
        printMessage(_("Your skin turns into steel!"));
    }
    ...
    if (py.flags.invulnerability == 0) {
        ...
        printCharacterCurrentArmorClass();
        printMessage(_("Your skin returns to normal."));
    }
}
...
static void playerUpdateBlessedness() {
    ...
    if ((py.flags.status & config::player::status::PY_BLESSED) == 0) {
        ...
        printMessage(_("You feel righteous!"));
        printCharacterCurrentArmorClass();
    }
    ...
    if (py.flags.blessed == 0) {
        ...
        printMessage(_("The prayer has expired."));
        printCharacterCurrentArmorClass();
    }
}
...
static void playerUpdateHeatResistance() {
    ...
    if (py.flags.heat_resistance == 0) {
        printMessage(_("You no longer feel safe from flame."));
    }
}
...
static void playerUpdateColdResistance() {
    ...
    if (py.flags.cold_resistance == 0) {
        printMessage(_("You no longer feel safe from cold."));
    }
}
...
static void playerUpdateWordOfRecall() {
    ...
    if (py.flags.word_of_recall == 1) {
        ...
        if (dg.current_level > 0) {
            dg.current_level = 0;
            printMessage(_("You feel yourself yanked upwards!"));
        } else if (py.misc.max_dungeon_depth != 0) {
            dg.current_level = py.misc.max_dungeon_depth;
            printMessage(_("You feel yourself yanked downwards!"));
        }
    } else {
...
static void playerDetectEnchantment() {
    for (int i = 0; i < PLAYER_INVENTORY_SIZE; i++) {
        ...
        if (item.category_id != TV_NOTHING && itemEnchanted(item) && randomNumber(chance) == 1) {
            vtype_t tmp_str = {'\0'};
            (void) snprintf(tmp_str, MORIA_MESSAGE_SIZE, _("There's something about what you are %s..."), playerItemWearingDescription(i));
            playerDisturb(0, 0);
...
static int getCommandRepeatCount(char &last_input_command) {
    const char *repeat_count_prompt = _("Repeat count:");
    putStringClearToEOL(repeat_count_prompt, Coord_t{0, 0});

    // Column where the digit entry field starts, derived from the display
    // width of the (translatable, e.g. Japanese) prompt above plus one
    // space gap -- not a hardcoded literal -- so a translated prompt of a
    // different display width than the English original never overlaps
    // with the digits drawn after it. `command_hint_column` preserves the
    // original code's fixed offset from the digit field (the "Command:"
    // hint intentionally overwrites the tail of the 7-character digit
    // field once digit entry has finished).
    const int digit_field_column = displayWidth(repeat_count_prompt) + 1;
    const int command_hint_column = digit_field_column + 6;
    ...
    while (true) {
        if (last_input_command == DELETE || last_input_command == CTRL_KEY('H')) {
            ...
            putStringClearToEOL(text_buffer, Coord_t{0, digit_field_column});
    ...
    if (last_input_command == ' ') {
        putStringClearToEOL(_("Command:"), Coord_t{0, command_hint_column});
        last_input_command = getKeyInput();
    }
...
static char parseAlternateCtrlInput(char last_input_command) {
    ...
    if (getCommand(_("Control-"), last_input_command)) {
        if (last_input_command >= 'A' && last_input_command <= 'Z') {
            ...
            last_input_command = ' ';
            printMessage(_("Type ^ <letter> for a control char"));
        }
    } else {
        last_input_command = ' ';
    }
        ...
        if (game.command_count > 0) {
            ...
            if (repeat_count > 0) {
                if (!validCountCommand(last_input_command)) {
                    ...
                    last_input_command = ' ';
                    printMessage(_("Invalid command with a count."));
                } else {
...
static void commandQuit() {
    ...
    if (getInputConfirmation(_("Do you really want to quit?"))) {
        game.character_is_dead = true;
        dg.generate_new_level = true;

        (void) strcpy(game.character_died_from, _("Quitting"));
    }
}
...
static void commandFlipWizardMode() {
    if (game.wizard_mode) {
        ...
        printMessage(_("Wizard mode off."));
    } else if (enterWizardMode()) {
        printMessage(_("Wizard mode on."));
    }
...
static void commandSaveAndExit() {
    if (game.total_winner) {
        printMessage(_("You are a Total Winner,  your character must be retired."));

        if (config::options::use_roguelike_keys) {
            printMessage(_("Use 'Q' to when you are ready to quit."));
        } else {
            printMessage(_("Use <Control>-K when you are ready to quit."));
        }
    } else {
        (void) strcpy(game.character_died_from, _("(saved)"));
        printMessage(_("Saving game..."));

        if (saveGame()) {
            endGame();
        }

        (void) strcpy(game.character_died_from, _("(alive and well)"));
    }
}

static void commandLocateOnMap() {
    if (py.flags.blind > 0 || playerNoLight()) {
        printMessage(_("You can't see your map."));
        return;
    }
    ...
    while (true) {
        ...
        if (panel.y == old_panel.y && panel.x == old_panel.x) {
            ...
            (void) snprintf(tmp_str, MORIA_MESSAGE_SIZE, //
                            _("%s%s of"),                //
                            panel.y < old_panel.y   ? _(" North")
                            : panel.y > old_panel.y ? _(" South")
                                                    : "", //
                            panel.x < old_panel.x   ? _(" West")
                            : panel.x > old_panel.x ? _(" East")
                                                    : "" //
            );
        }

        (void) snprintf(out_val, MORIA_MESSAGE_SIZE, _("Map sector [%d,%d], which is%s your sector. Look which direction?"), panel.y, panel.x, tmp_str);
        ...
        while (true) {
            ...
            if (player_coord.x < 0 || player_coord.y < 0 || player_coord.x >= dg.width || player_coord.y >= dg.width) {
                printMessage(_("You've gone past the end of your map."));
...
static void doWizardCommands(char command) {
    switch (command) {
        ...
        default:
            if (config::options::use_roguelike_keys) {
                putStringClearToEOL(_("Type '?' or '\\' for help."), Coord_t{0, 0});
            } else {
                putStringClearToEOL(_("Type '?' or ^H for help."), Coord_t{0, 0});
            }
    }
}
    ...
    switch (command) {
        ...
        default:
            ...
            if (game.wizard_mode) {
                ...
                putStringClearToEOL(_("Type '?' for help."), Coord_t{0, 0});
            }
    }
...
static void examineBook() {
    ...
    if (!inventoryFindRange(TV_MAGIC_BOOK, TV_PRAYER_BOOK, item_pos_start, item_pos_end)) {
        printMessage(_("You are not carrying any books."));
        return;
    }

    if (py.flags.blind > 0) {
        printMessage(_("You can't see to read your spell book!"));
        return;
    }

    if (playerNoLight()) {
        printMessage(_("You have no light to read by."));
        return;
    }

    if (py.flags.confused > 0) {
        printMessage(_("You are too confused."));
        return;
    }

    int item_id;
    if (inventoryGetInputForItemId(item_id, _("Which Book?"), item_pos_start, item_pos_end, CNIL, CNIL)) {
        int spell_index[31];
        bool can_read = true;
        ...
        if (!can_read) {
            printMessage(_("You do not understand the language."));
            return;
        }
...
static void dungeonGoUpLevel() {
    ...
    if (tile_id != 0 && game.treasure.list[tile_id].category_id == TV_UP_STAIR) {
        ...
        printMessage(_("You enter a maze of up staircases."));
        printMessage(_("You pass through a one-way door."));

        dg.generate_new_level = true;
    } else {
        printMessage(_("I see no up staircase here."));
        game.player_free_turn = true;
    }
}
...
static void dungeonGoDownLevel() {
    ...
    if (tile_id != 0 && game.treasure.list[tile_id].category_id == TV_DOWN_STAIR) {
        ...
        printMessage(_("You enter a maze of down staircases."));
        printMessage(_("You pass through a one-way door."));

        dg.generate_new_level = true;
    } else {
        printMessage(_("I see no down staircase here."));
        game.player_free_turn = true;
    }
}
...
static void dungeonJamDoor() {
    ...
    if (tile.treasure_id == 0) {
        printMessage(_("That isn't a door!"));
        return;
    }
    ...
    if (item_id == TV_OPEN_DOOR) {
        printMessage(_("The door must be closed first."));
        return;
    }
    ...
    if (tile.creature_id == 0) {
        ...
        if (inventoryFindRange(TV_SPIKE, TV_NEVER, item_pos_start, item_pos_end)) {
            ...
            printMessageNoCommandInterrupt(_("You jam the door with a spike."));
            ...
            printMessage(_("But you have no spikes."));
        }
    } else {
        game.player_free_turn = false;

        vtype_t msg = {'\0'};
        (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("The %s is in your way!"), _(creatures_list[monsters[tile.creature_id].creature_id].name));
        printMessage(msg);
    }
}
...
static void inventoryRefillLamp() {
    ...
    if (py.inventory[PlayerEquipment::Light].sub_category_id != 0) {
        printMessage(_("But you are not using a lamp."));
        return;
    }

    int item_pos_start, item_pos_end;
    if (!inventoryFindRange(TV_FLASK, TV_NEVER, item_pos_start, item_pos_end)) {
        printMessage(_("You have no oil."));
        return;
    }
    ...
    if (item.misc_use > config::treasure::OBJECT_LAMP_MAX_CAPACITY) {
        ...
        printMessage(_("Your lamp overflows, spilling oil on the ground."));
        printMessage(_("Your lamp is full."));
    } else if (item.misc_use > config::treasure::OBJECT_LAMP_MAX_CAPACITY / 2) {
        printMessage(_("Your lamp is more than half full."));
    } else if (item.misc_use == config::treasure::OBJECT_LAMP_MAX_CAPACITY / 2) {
        printMessage(_("Your lamp is half full."));
    } else {
        printMessage(_("Your lamp is less than half full."));
    }
