...
void recordNewHighScore() {
    ...
    if (panic_save) {
        printMessage(_("Sorry, scores for games restored from panic save files are not saved."));
        return;
    }
    ...
    if ((highscore_fp = fopen(config::files::scores.c_str(), "rb+")) == nullptr) {
        printMessage((_("Error opening score file '") + config::files::scores + _("'.")).c_str());
        printMessage(CNIL);
        return;
    }
    ...
    while (feof(highscore_fp) == 0) {
        ...
        if (((new_entry.uid != 0 && new_entry.uid == old_entry.uid) ||
             (new_entry.uid == 0 && (strcmp(old_entry.died_from, _("(saved)")) == 0) && new_entry.birth_date == old_entry.birth_date)) &&
...
void showScoresScreen() {
    ...
    if (feof(highscore_fp) == 0 && !validGameVersion(version_maj, version_min, patch_level)) {
        printMessage(_("Sorry. This score file is from a different version of umoria."));
        printMessage(CNIL);
        (void) fclose(highscore_fp);
        return;
    }
    ...
    // race/class/died_from can all be translated (Japanese) text, so
    // they're padded to a target *display column* width, not a byte
    // count -- a raw `%-N.Ns` printf specifier would count bytes and
    // could cut a multi-byte UTF-8 character in half. The line is built
    // as a std::string, not a fixed-size char buffer, since multi-byte
    // characters make the worst-case byte length larger than the
    // display-column budget (e.g. 10 display columns of Japanese text
    // can be 30 bytes).
    char prefix[16];
    char level_field[8];
    ...
    while (feof(highscore_fp) == 0) {
        ...
        while ((feof(highscore_fp) == 0) && i < 21) {
            (void) snprintf(prefix, sizeof(prefix), "%-4d%8d ", rank, score.points);
            (void) snprintf(level_field, sizeof(level_field), "%3d ", score.level);

            std::string msg = std::string(prefix) +                                            //
                              padToDisplayWidth(score.name, 19) + " " +                        //
                              std::string(1, (char) score.gender) + " " +                      //
                              padToDisplayWidth(_(character_races[score.race].name), 10) +     //
                              " " +                                                            //
                              padToDisplayWidth(_(classes[score.character_class].title), 10) + //
                              level_field +                                                    //
                              padToDisplayWidth(score.died_from, 22);
            i++;
            putStringClearToEOL(msg, Coord_t{i, 0});
            rank++;
            readHighScore(score);
        }
        putStringClearToEOL(_("Rank  Points Name              Sex Race       Class     Lvl Killed By"), Coord_t{0, 0});
