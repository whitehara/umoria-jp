...
static bool playerCanPray(int &item_pos_begin, int &item_pos_end) {
    if (py.flags.blind > 0) {
        printMessage(_("You can't see to read your prayer!"));
        return false;
    }
    ...
    if (classes[py.misc.class_id].class_to_use_mage_spells != config::spells::SPELL_TYPE_PRIEST) {
        printMessage(_("Pray hard enough and your prayers may be answered."));
        return false;
    }
    ...
    if (!inventoryFindRange(TV_PRAYER_BOOK, TV_NEVER, item_pos_begin, item_pos_end)) {
        printMessage(_("You are not carrying any Holy Books!"));
        return false;
    }
...
static void playerRecitePrayer(int prayer_type) {
    ...
    switch ((PriestSpellTypes) (prayer_type + 1)) {
        ...
        case PriestSpellTypes::OrbOfDraining:
            if (getDirectionWithMemory(CNIL, dir)) {
                spellFireBall(py.pos, dir, (diceRoll(Dice_t{3, 6}) + py.misc.level), MagicSpellFlags::HolyOrb, _("Black Sphere"));
            }
            break;
...
void pray() {
    ...
    int item_id;
    if (!inventoryGetInputForItemId(item_id, _("Use which Holy Book?"), item_pos_begin, item_pos_end, CNIL, CNIL)) {
        return;
    }

    int choice, chance;
    int result = castSpellGetId(_("Recite which prayer?"), item_id, choice, chance);
    if (result < 0) {
        printMessage(_("You don't know any prayers in that book."));
        return;
    }
    if (result == 0) {
        return;
    ...
    if (randomNumber(100) < chance) {
        printMessage(_("You lost your concentration!"));
    } else {
    ...
    if (spell.mana_required > py.misc.current_mana) {
        printMessage(_("You faint from fatigue!"));
        ...
        if (randomNumber(3) == 1) {
            printMessage(_("You have damaged your health!"));
