...
static void inventoryItemWeightText(obj_desc_t text, int itemId) {
    ...
    (void) snprintf(text, MORIA_OBJ_DESC_SIZE, _("%3d.%d lb"), quotient, remainder);
}
...
int displayInventoryItems(int itemIdStart, int itemIdEnd, bool weighted, int column, const char *mask) {
    ...
    for (int i = itemIdStart; i <= itemIdEnd; i++) {
        ...
        // Truncate if too long. (by display column, not byte count, so a
        // multi-byte, e.g. Japanese, item name is never cut in the middle.)
        std::string truncated_description = truncateToWidth(description, lim);
        (void) strncpy(description, truncated_description.c_str(), sizeof(obj_desc_t) - 1);
        description[sizeof(obj_desc_t) - 1] = '\0';

        (void) snprintf(descriptions[i], MORIA_MESSAGE_SIZE, _("%c) %s"), 'a' + i, description);

        // Column width budget, not byte count: `descriptions[i]` can
        // contain translated (e.g. Japanese) item names, where multi-byte
        // characters make strlen() overcount relative to display columns,
        // pushing this panel further left (or even off-screen) than
        // necessary.
        int l = displayWidth(descriptions[i]) + 2;
...
const char *playerItemWearingDescription(int body_location) {
    switch (body_location) {
        ...
        case PlayerEquipment::Head:
            return _("wearing on your head");
        case PlayerEquipment::Neck:
            return _("wearing around your neck");
        case PlayerEquipment::Body:
            return _("wearing on your body");
        case PlayerEquipment::Arm:
            return _("wearing on your arm");
        case PlayerEquipment::Hands:
            return _("wearing on your hands");
        case PlayerEquipment::Right:
            return _("wearing on your right hand");
        case PlayerEquipment::Left:
            return _("wearing on your left hand");
        case PlayerEquipment::Feet:
            return _("wearing on your feet");
        case PlayerEquipment::Outer:
            return _("wearing about your body");
        case PlayerEquipment::Light:
            return _("using to light the way");
        case PlayerEquipment::Auxiliary:
            return _("holding ready by your side");
        default:
            return _("carrying in your pack");
    }
}
    ...
    switch (id) {
        case PlayerEquipment::Wield:
            if (py.stats.used[PlayerAttr::A_STR] * 15 < weight) {
                return _("Just lifting");
            }
            return _("Wielding");
        case PlayerEquipment::Head:
            return _("On head");
        case PlayerEquipment::Neck:
            return _("Around neck");
        case PlayerEquipment::Body:
            return _("On body");
        case PlayerEquipment::Arm:
            return _("On arm");
        case PlayerEquipment::Hands:
            return _("On hands");
        case PlayerEquipment::Right:
            return _("On right hand");
        case PlayerEquipment::Left:
            return _("On left hand");
        case PlayerEquipment::Feet:
            return _("On feet");
        case PlayerEquipment::Outer:
            return _("About body");
        case PlayerEquipment::Light:
            return _("Light source");
        case PlayerEquipment::Auxiliary:
            return _("Spare weapon");
        default:
            return _("Unknown equipment position ID");
    }
}
...
int displayEquipment(bool showWeights, int column) {
    ...
    for (int i = PlayerEquipment::Wield; i < PLAYER_INVENTORY_SIZE; i++) {
        ...
        // Truncate if necessary (by display column, not byte count, so a
        // multi-byte, e.g. Japanese, item name is never cut in the middle.)
        std::string truncated_description = truncateToWidth(description, lim);
        (void) strncpy(description, truncated_description.c_str(), sizeof(obj_desc_t) - 1);
        description[sizeof(obj_desc_t) - 1] = '\0';

        (void) snprintf(descriptions[line], MORIA_MESSAGE_SIZE, _("%c) %-14s: %s"), line + 'a', equippedDescription, description);

        // Column width budget, not byte count: `descriptions[line]` can
        // contain translated (e.g. Japanese) item/equipment-position text,
        // where multi-byte characters make strlen() overcount relative to
        // display columns, pushing this panel further left than necessary.
        int l = displayWidth(descriptions[line]) + 2;
...
static int showEquipmentHelpMenu(int leftColumn) {
    ...
    putStringClearToEOL(_("  ESC: exit"), Coord_t{1, leftColumn});
    putStringClearToEOL(_("  w  : wear or wield object"), Coord_t{2, leftColumn});
    putStringClearToEOL(_("  t  : take off item"), Coord_t{3, leftColumn});
    putStringClearToEOL(_("  d  : drop object"), Coord_t{4, leftColumn});
    putStringClearToEOL(_("  x  : exchange weapons"), Coord_t{5, leftColumn});
    putStringClearToEOL(_("  i  : inventory of pack"), Coord_t{6, leftColumn});
    putStringClearToEOL(_("  e  : list used equipment"), Coord_t{7, leftColumn});
...
static bool verifyAction(const char *prompt, int item) {
    ...
    replaceLastCharSafely(description, '?');

    obj_desc_t msg = {'\0'};
    (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("%s %s"), prompt, description);
...
static void requestAndShowInventoryScreen(bool recoverScreen) {
    ...
    if (screen_has_changed) {
        if (recoverScreen || !getInputConfirmation(_("Continuing with inventory command?"))) {
...
static bool uiCommandInventoryTakeOffItem(bool selecting) {
    if (py.equipment_count == 0) {
        printMessage(_("You are not using any equipment."));
    ...
    if (py.pack.unique_items >= PlayerEquipment::Wield && (game.doing_inventory_command == 0)) {
        printMessage(_("You will have to drop something first."));
        return selecting;
    }
...
static bool uiCommandInventoryDropItem(char &command, bool selecting) {
    if (py.pack.unique_items == 0 && py.equipment_count == 0) {
        printMessage(_("But you're not carrying anything."));
        return selecting;
    }

    if (dg.floor[py.pos.y][py.pos.x].treasure_id != 0) {
        printMessage(_("There's no room to drop anything here."));
        return selecting;
    }
...
static bool uiCommandInventoryWearWieldItem(bool selecting) {
    ...
    if (game.screen.wear_low_id > game.screen.wear_high_id) {
        printMessage(_("You have nothing to wear or wield."));
        return selecting;
    }
...
static void uiCommandInventoryUnwieldItem() {
    if (!playerIsWieldingItem()) {
        printMessage(_("But you are wielding no weapons."));
        return;
    }
    ...
    if (playerWornItemIsCursed(PlayerEquipment::Wield)) {
        ...
        obj_desc_t msg = {'\0'};
        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("The %s you are wielding appears to be cursed."), description);
    ...
    if (py.inventory[PlayerEquipment::Wield].category_id != TV_NOTHING) {
        obj_desc_t label = {'\0'};
        (void) strcpy(label, _("Primary weapon   : "));
        ...
        printMessage(_("No primary weapon."));
    }
...
static void buildCommandHeading(obj_desc_t str, int from, int to, const char *swap, char command, const char *prompt) {
    ...
    (void) snprintf(str, MORIA_OBJ_DESC_SIZE, _("(%c-%c%s%s%s, space to break, ESC to exit) %s which one?"), from, to, list, swap, digits, prompt);
}
...
static int requestPutRingOnWhichHand() {
    ...
    do {
        char query;
        if (!getMenuItemId(_("Put ring on which hand (l/r/L/R)?"), query)) {
            hand = -1;
        } else if (query == 'l') {
            ...
            if ((hand != 0) && !verifyAction(_("Replace"), hand)) {
                hand = 0;
            }
        }
    } while (hand == 0);
...
static int inventoryGetSlotToWearEquipment(int categoryId) {
    ...
    switch (categoryId) {
        ...
        case TV_RING:
            ...
            break;
        default:
            printMessage(_("IMPOSSIBLE: I don't see how you can use that."));
            break;
    }
...
static void inventoryItemIsCursedMessage(int itemId) {
    ...
    obj_desc_t msg = {'\0'};
    (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("The %s you are "), description);

    if (itemId == PlayerEquipment::Head) {
        (void) strcat(msg, _("wielding "));
    } else {
        (void) strcat(msg, _("wearing "));
    }

    printMessage(strcat(msg, _("appears to be cursed.")));
}
...
static bool executeRemoveItemCommand(bool selecting, int itemId, char &command, const char *which, const char *prompt) {
    ...
    if ((isupper((int) *which) != 0) && !verifyAction((char *) prompt, itemId)) {
        ...
        itemId = -1;
        printMessage(_("Hmmm, it seems to be cursed."));
    } else if (command == 't' && !inventoryCanCarryItemCount(py.inventory[itemId])) {
        if (dg.floor[py.pos.y][py.pos.x].treasure_id != 0) {
            itemId = -1;
            printMessage(_("You can't carry it."));
        } else if (getInputConfirmation(_("You can't carry it.  Drop it?"))) {
            command = 'r';
        } else {
            itemId = -1;
        }
...
static void executeWearItemCommand(int itemId, const char *which, const char *prompt) {
    ...
    *item = savedItem;
    ...
    if (slot == PlayerEquipment::Wield) {
        text = _("You are wielding");
    } else if (slot == PlayerEquipment::Light) {
        text = _("Your light source is");
    } else {
        text = _("You are wearing");
    }
    ...
    obj_desc_t msg = {'\0'};
    (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("%s %s (%c)"), text, description, 'a' + itemId);
    printMessage(msg);
    ...
    if (inventoryItemIsCursed(*item)) {
        printMessage(_("Oops! It feels deathly cold!"));
...
static void executeDropItemCommand(int itemId, const char *which, const char *prompt) {
    ...
    if (py.inventory[itemId].items_count > 1) {
        ...
        replaceLastCharSafely(description, '?'); // replace period with question

        obj_desc_t msg = {'\0'};
        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("Drop all %s"), description);
...
static void inventoryDisplayAppropriateHeader() {
    if (game.screen.current_screen_id == Screen::Inventory) {
        ...
        if (!config::options::show_inventory_weights || py.pack.unique_items == 0) {
            (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("You are carrying %d.%d pounds. In your pack there is %s"), //
                            weightQuotient, weightRemainder, (py.pack.unique_items == 0 ? _("nothing.") : "-"));
        } else {
            ...
            (void) snprintf(msg, MORIA_OBJ_DESC_SIZE,                                                             //
                            _("You are carrying %d.%d pounds. Your capacity is %d.%d pounds. In your pack is -"), //
        ...
        if (game.screen.wear_high_id < game.screen.wear_low_id) {
            putStringClearToEOL(_("You have nothing you could wield."), Coord_t{0, 0});
        } else {
            putStringClearToEOL(_("You could wield -"), Coord_t{0, 0});
        }
    } else if (game.screen.current_screen_id == Screen::Equipment) {
        if (py.equipment_count == 0) {
            putStringClearToEOL(_("You are not using anything."), Coord_t{0, 0});
        } else {
            putStringClearToEOL(_("You are using -"), Coord_t{0, 0});
        }
    } else {
        putStringClearToEOL(_("Allowed commands:"), Coord_t{0, 0});
    }
...
static void uiCommandDisplayInventory() {
    if (py.pack.unique_items == 0) {
        printMessage(_("You are not carrying anything."));
    } else {
        uiCommandSwitchScreen(Screen::Inventory);
    }
}
...
void inventoryExecuteCommand(char command) {
    ...
    do {
        ...
        if (which == ESCAPE || game.screen.current_screen_id == Screen::Blank) {
            ...
            putString(_("e/i/t/w/x/d/?/ESC:"), Coord_t{game.screen.screen_bottom_pos, 60});
            command = getKeyInput();
...
static bool inventorySwitchPackMenu(vtype_t &prompt, PackMenu &menu, bool menuActive, int &itemIdEnd) {
    if (menu == PackMenu::Inventory) {
        ...
        if (py.equipment_count == 0) {
            putStringClearToEOL(_("But you're not using anything -more-"), Coord_t{0, 0});
            (void) getKeyInput();
        } else {
    ...
    if (py.pack.unique_items == 0) {
        putStringClearToEOL(_("But you're not carrying anything -more-"), Coord_t{0, 0});
        (void) getKeyInput();
        return false;
    }
...
bool inventoryGetInputForItemId(int &commandKeyId, const char *prompt, int itemIdStart, int itemIdEnd, char *mask, const char *message) {
    ...
    if (py.pack.unique_items < 1 && (!packFull || py.equipment_count < 1)) {
        putStringClearToEOL(_("You are not carrying anything."), Coord_t{0, 0});
        return false;
    }
    ...
    do {
        ...
        if (packFull) {
            (void) snprintf(description, MORIA_MESSAGE_SIZE,                         //
                            _("(%s: %c-%c,%s%s / for %s, or ESC) %s"),               //
                            (menu == PackMenu::Inventory ? _("Inven") : _("Equip")), //
                            itemIdStart + 'a',                                       //
                            itemIdEnd + 'a',                                         //
                            (menu == PackMenu::Inventory ? _(" 0-9,") : ""),         //
                            (menuActive ? "" : _(" * to see,")),                     //
                            (menu == PackMenu::Inventory ? _("Equip") : _("Inven")), //
                            prompt                                                   //
            );
        } else {
            (void) snprintf(description, MORIA_MESSAGE_SIZE,                 //
                            _("(Items %c-%c,%s%s ESC to exit) %s"),          //
                            itemIdStart + 'a',                               //
                            itemIdEnd + 'a',                                 //
                            (menu == PackMenu::Inventory ? _(" 0-9,") : ""), //
                            (menuActive ? "" : _(" * for inventory list,")), //
                            prompt                                           //
            );
        }
            ...
            switch (which) {
                ...
                default:
                    ...
                    if (commandKeyId >= itemIdStart && commandKeyId <= itemIdEnd && (mask == CNIL || (mask[commandKeyId] != 0))) {
                        ...
                        if ((isupper((int) which) != 0) && !verifyAction(_("Try"), commandKeyId)) {
                            menu = PackMenu::CloseMenu;
                            done = true;
