...
static void monsterPrintAttackDescription(char *msg, int attack_id) {
    switch (attack_id) {
        case 1:
            printMessage(strcat(msg, _("hits you.")));
            break;
        case 2:
            printMessage(strcat(msg, _("bites you.")));
            break;
        case 3:
            printMessage(strcat(msg, _("claws you.")));
            break;
        case 4:
            printMessage(strcat(msg, _("stings you.")));
            break;
        case 5:
            printMessage(strcat(msg, _("touches you.")));
            break;
#if 0
        case 6:
                    msg_print(strcat(msg, _("kicks you.")));
                    break;
#endif
        case 7:
            printMessage(strcat(msg, _("gazes at you.")));
            break;
        case 8:
            printMessage(strcat(msg, _("breathes on you.")));
            break;
        case 9:
            printMessage(strcat(msg, _("spits on you.")));
            break;
        case 10:
            printMessage(strcat(msg, _("makes a horrible wail.")));
            break;
#if 0
        case 11:
                    msg_print(strcat(msg, _("embraces you.")));
                    break;
#endif
        case 12:
            printMessage(strcat(msg, _("crawls on you.")));
            break;
        case 13:
            printMessage(strcat(msg, _("releases a cloud of spores.")));
            break;
        case 14:
            printMessage(strcat(msg, _("begs you for money.")));
            break;
        case 15:
            printMessage(_("You've been slimed!"));
            break;
        case 16:
            printMessage(strcat(msg, _("crushes you.")));
            break;
        case 17:
            printMessage(strcat(msg, _("tramples you.")));
            break;
        case 18:
            printMessage(strcat(msg, _("drools on you.")));
            break;
        case 19:
            switch (randomNumber(9)) {
                case 1:
                    printMessage(strcat(msg, _("insults you!")));
                    break;
                case 2:
                    printMessage(strcat(msg, _("insults your mother!")));
                    break;
                case 3:
                    printMessage(strcat(msg, _("gives you the finger!")));
                    break;
                case 4:
                    printMessage(strcat(msg, _("humiliates you!")));
                    break;
                case 5:
                    printMessage(strcat(msg, _("wets on your leg!")));
                    break;
                case 6:
                    printMessage(strcat(msg, _("defiles you!")));
                    break;
                case 7:
                    printMessage(strcat(msg, _("dances around you!")));
                    break;
                case 8:
                    printMessage(strcat(msg, _("makes obscene gestures!")));
                    break;
                case 9:
                    printMessage(strcat(msg, _("moons you!!!")));
                    break;
                default:
                    break;
            }
            break;
        case 99:
            printMessage(strcat(msg, _("is repelled.")));
            break;
        default:
            break;
    }
...
static void monsterConfuseOnAttack(Creature_t const &creature, Monster_t &monster, int attack_type, vtype_t monster_name, bool visible) {
    if (py.flags.confuse_monster && attack_type != 99) {
        printMessage(_("Your hands stop glowing."));
        py.flags.confuse_monster = false;

        vtype_t msg = {'\0'};

        if (randomNumber(MON_MAX_LEVELS) < creature.level || ((creature.defenses & config::monsters::defense::CD_NO_SLEEP) != 0)) {
            (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("%sis unaffected."), monster_name);
        } else {
            (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("%sappears confused."), monster_name);
...
static void monsterAttackPlayer(int monster_id) {
    ...
    vtype_t name = {'\0'};
    if (!monster.lit) {
        (void) strcpy(name, _("It "));
    } else if (lang::currentLanguage() == "ja") {
        // Japanese has no articles: bare name only. Kept trailing-space
        // to match the buffer convention the strcat() calls below rely
        // on; natural Japanese phrasing for this whole sentence is a
        // future translation-data concern, not this mechanism's job.
        (void) snprintf(name, MORIA_MESSAGE_SIZE, "%s ", _(creature.name));
    } else {
        (void) snprintf(name, MORIA_MESSAGE_SIZE, _("The %s "), _(creature.name));
    }

    vtype_t death_description = {'\0'};
    playerDiedFromString(&death_description, _(creature.name), creature.movement);
    ...
    for (auto &damage_type_id : creature.damage) {
        ...
        if (playerTestAttackHits(attack_type, creature.level)) {
            ...
            if ((attack_desc >= 1 && attack_desc <= 3) || attack_desc == 6) {
                ...
                vtype_t description = {'\0'};
                (void) strcpy(description, name);
                printMessage(strcat(description, _("misses you.")));
            }
        }
    ...
    if ((move_bits & config::monsters::move::CM_OPEN_DOOR) != 0u) {
        ...
        if (item.category_id == TV_CLOSED_DOOR) {
            ...
            if (item.misc_use == 0) {
                ...
                if (randomNumber((monster_hp + 1) * (50 - item.misc_use)) < 40 * (monster_hp - 10 + item.misc_use)) {
                    printMessage(_("You hear a door burst open!"));
                    playerDisturb(1, 0);
                    door_is_stuck = true;
                    do_move = true;
                }
...
static void glyphOfWardingProtection(uint16_t creature_id, uint32_t move_bits, bool &do_move, bool &do_turn, Coord_t coord) {
    if (randomNumber(config::treasure::OBJECTS_RUNE_PROTECTION) < creatures_list[creature_id].level) {
        if (coord.y == py.pos.y && coord.x == py.pos.x) {
            printMessage(_("The rune of protection is broken!"));
        }
    ...
    switch (spell_id) {
        ...
        case 8: // Light Wound
            if (playerSavingThrow()) {
                printMessage(_("You resist the effects of the spell."));
            } else {
        ...
        case 10: // Hold Person
            if (py.flags.free_action) {
                printMessage(_("You are unaffected."));
        ...
        case 14: // Summon Monster
            (void) strcat(monster_name, _("magically summons a monster!"));
        ...
        case 15: // Summon Undead
            (void) strcat(monster_name, _("magically summons an undead!"));
            printMessage(monster_name);
            coord.y = py.pos.y;
            coord.x = py.pos.x;
        ...
        case 17: // Drain Mana
            if (py.misc.current_mana > 0) {
                ...
                vtype_t msg = {'\0'};
                (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("%sdraws psychic energy from you!"), monster_name);
                printMessage(msg);

                if (monster.lit) {
                    (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("%sappears healthier."), monster_name);
                    printMessage(msg);
                }
...
static bool monsterCastSpell(int monster_id) {
    ...
    // Describe the attack
    vtype_t name = {'\0'};
    if (monster.lit && lang::currentLanguage() == "ja") {
        // Japanese has no articles: bare name only. Kept trailing-space
        // to match the buffer convention the strcat() calls below rely
        // on; natural Japanese phrasing for this whole sentence is a
        // future translation-data concern, not this mechanism's job.
    ...
    if ((thrown_spell < 14 && thrown_spell > 6) || thrown_spell == 16) {
        (void) strcat(name, _("casts a spell."));
        printMessage(name);
    }
...
static void monsterMoveOutOfWall(Monster_t const &monster, int monster_id, uint32_t &rcmove) {
    ...
    if (dg.floor[monster.pos.y][monster.pos.x].feature_id >= MIN_CAVE_WALL) {
        ...
        if (i >= 0) {
            printMessage(_("You hear a scream muffled by rock!"));
            displayCharacterExperience();
        } else {
            printMessage(_("A creature digs itself out from the rock!"));
        ...
        if (monster.lit || monster.distance_from_player <= creatures_list[monster.creature_id].area_affect_radius ||
            ...
            if (monster.stunned_amount != 0) {
                ...
                if (monster.stunned_amount == 0) {
                    if (monster.lit) {
                        vtype_t msg = {'\0'};
                        (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("The %s "), _(creatures_list[monster.creature_id].name));
                        printMessage(strcat(msg, _("recovers and glares at you.")));
                    }
                }
            }
...
uint32_t monsterDeath(Coord_t coord, uint32_t flags) {
    ...
    if (((flags & config::monsters::move::CM_WIN) != 0u) && !game.character_is_dead) {
        ...
        printMessage(_("*** CONGRATULATIONS *** You have won the game."));
        printMessage(_("You cannot save this game, but you may retire when ready."));
    }
...
void printMonsterActionText(const std::string &name, const std::string &action) {
    printMessage(lang::format(_("%s %s"), {name, action}).c_str());
}

std::string monsterNameDescription(const std::string &real_name, bool is_lit) {
    if (is_lit) {
        return lang::format(_("The %s"), {_(real_name)});
    }
    return _("It");
}
    ...
    for (int y = coord.y - 1; y <= coord.y + 1 && y < MAX_HEIGHT; y++) {
        for (int x = coord.x - 1; x <= coord.x + 1 && x < MAX_WIDTH; x++) {
            ...
            if (randomNumber(MON_MAX_LEVELS) < creature.level || ((creature.defenses & config::monsters::defense::CD_NO_SLEEP) != 0)) {
                ...
                printMonsterActionText(name, _("is unaffected."));
            } else {
                monster.sleep_count = 500;
                asleep = true;

                printMonsterActionText(name, _("falls asleep."));
            }
        }
    }
    ...
    switch (attack_type) {
        ...
        case 2: // Lose Strength
            ...
            if (py.flags.sustain_str) {
                printMessage(_("You feel weaker for a moment, but it passes."));
            } else if (randomNumber(2) == 1) {
                printMessage(_("You feel weaker."));
        ...
        case 3: // Confusion attack
            ...
            if (randomNumber(2) == 1) {
                if (py.flags.confused < 1) {
                    printMessage(_("You feel confused."));
        ...
        case 4: // Fear attack
            playerTakesHit(damage, death_description);
            if (playerSavingThrow()) {
                printMessage(_("You resist the effects!"));
            } else if (py.flags.afraid < 1) {
                printMessage(_("You are suddenly afraid!"));
            ...
            break;
        case 5: // Fire attack
            printMessage(_("You are enveloped in flames!"));
            damageFire(damage, death_description);
            break;
        case 6: // Acid attack
            printMessage(_("You are covered in acid!"));
            damageAcid(damage, death_description);
            break;
        case 7: // Cold attack
            printMessage(_("You are covered with frost!"));
            damageCold(damage, death_description);
            break;
        case 8: // Lightning attack
            printMessage(_("Lightning strikes you!"));
            damageLightningBolt(damage, death_description);
            break;
        case 9: // Corrosion attack
            printMessage(_("A stinging red gas swirls about you."));
        ...
        case 10: // Blindness attack
            ...
            if (py.flags.blind < 1) {
                ...
                printMessage(_("Your eyes begin to sting."));
            } else {
                py.flags.blind += 5;
                noticed = false;
            }
        ...
        case 11: // Paralysis attack
            ...
            if (playerSavingThrow()) {
                ...
                if (py.flags.free_action) {
                    ...
                    printMessage(_("You are paralyzed."));
                }
            } else {
                noticed = false;
            }
        ...
        case 12: // Steal Money
            if (py.flags.paralysis < 1 && randomNumber(124) < py.stats.used[PlayerAttr::A_DEX]) {
                printMessage(_("You quickly protect your money pouch!"));
            } else {
                ...
                printMessage(_("Your purse feels lighter."));
                printCharacterGoldValue();
            }
            if (randomNumber(2) == 1) {
                printMessage(_("There is a puff of smoke!"));
                spellTeleportAwayMonster(monster_id, config::monsters::MON_MAX_SIGHT);
            }
            break;
        case 13: // Steal Object
            if (py.flags.paralysis < 1 && randomNumber(124) < py.stats.used[PlayerAttr::A_DEX]) {
                printMessage(_("You grab hold of your backpack!"));
            } else {
                inventoryDestroyItem(randomNumber(py.pack.unique_items) - 1);
                printMessage(_("Your backpack feels lighter."));
            }
            if (randomNumber(2) == 1) {
                printMessage(_("There is a puff of smoke!"));
                spellTeleportAwayMonster(monster_id, config::monsters::MON_MAX_SIGHT);
        ...
        case 14: // Poison
            playerTakesHit(damage, death_description);
            printMessage(_("You feel very sick."));
            py.flags.poisoned += randomNumber((int) creature_level) + 5;
            break;
        case 15: // Lose dexterity
            playerTakesHit(damage, death_description);
            if (py.flags.sustain_dex) {
                printMessage(_("You feel clumsy for a moment, but it passes."));
            } else {
                printMessage(_("You feel more clumsy."));
        ...
        case 16: // Lose constitution
            ...
            if (py.flags.sustain_con) {
                printMessage(_("Your body resists the effects of the disease."));
            } else {
                printMessage(_("Your health is damaged!"));
                (void) playerStatRandomDecrease(PlayerAttr::A_CON);
            }
            break;
        case 17: // Lose intelligence
            playerTakesHit(damage, death_description);
            printMessage(_("You have trouble thinking clearly."));
            if (py.flags.sustain_int) {
                printMessage(_("But your mind quickly clears."));
            } else {
        ...
        case 18: // Lose wisdom
            ...
            if (py.flags.sustain_wis) {
                printMessage(_("Your wisdom is sustained."));
            } else {
                printMessage(_("Your wisdom is drained."));
                (void) playerStatRandomDecrease(PlayerAttr::A_WIS);
            }
            break;
        case 19: // Lose experience
            printMessage(_("You feel your life draining away!"));
        ...
        case 21: // Disenchant
            if (executeDisenchantAttack()) {
                printMessage(_("There is a static feeling in the air."));
        ...
        case 22: // Eat food
            if (inventoryFindRange(TV_FOOD, TV_NEVER, item_pos_start, item_pos_end)) {
                ...
                printMessage(_("It got at your rations!"));
            } else {
                noticed = false;
            }
            break;
