...
// Column where the "yes"/"no " toggle text starts, both when the whole
// option line is redrawn and when just the toggle is overwritten in
// place (see setGameOptions() below). Derived from padding the option
// label to a fixed *display-column* width (not a byte-width %-38s
// printf specifier), so a translated (e.g. Japanese) label of a
...
void setGameOptions() {
    putStringClearToEOL(_("  ESC when finished, y/n to set options, <return> or - to move cursor"), Coord_t{0, 0});

    int max;
    for (max = 0; game_options[max].o_prompt != nullptr; max++) {
        std::string line = padToDisplayWidth(_(game_options[max].o_prompt), GAME_OPTION_LABEL_WIDTH) + ": " + (*game_options[max].o_var ? _("yes") : _("no "));
        putStringClearToEOL(line, Coord_t{max + 1, 0});
    }
    eraseLine(Coord_t{max + 1, 0});

    int option_id = 0;
    while (true) {
        moveCursor(Coord_t{option_id + 1, GAME_OPTION_VALUE_COLUMN});
