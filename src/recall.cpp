    ...
    while (*p != 0) {
        *roff_buffer_pointer = *p;
        ...
        if (*p == '\n' || roff_buffer_pointer >= roff_buffer + sizeof(roff_buffer) - 1) {
            ...
            if (*p != '\n') {
                // Walk back to the last space to break the line on a word
                // boundary -- but never past the start of `roff_buffer`.
                // Translated (e.g. Japanese) text has no space-delimited
                // words at all, so a run of 80 translated bytes with no
                // ASCII space used to walk `q` straight past the start of
                // the buffer with no bound, reading/comparing arbitrary
                // stack memory until it happened to find a stray 0x20 byte
                // (or crashed first). If no space is found, fall back to a
                // hard break at the buffer boundary instead.
                while (q > roff_buffer && *q != ' ') {
                    q--;
                }
                if (*q != ' ') {
                    q = roff_buffer_pointer;
                }
            }
            *q = 0;
...
static void memoryConflictHistory(uint16_t deaths, uint16_t kills) {
    ...
    if (deaths != 0u) {
        (void) snprintf(desc, MORIA_OBJ_DESC_SIZE, _("%d of the contributors to your monster memory %s"), deaths, plural(deaths, _("has"), _("have")));
        memoryPrint(desc);
        memoryPrint(_(" been killed by this creature, and "));
        if (kills == 0) {
            memoryPrint(_("it is not ever known to have been defeated."));
        } else {
            (void) snprintf(desc, MORIA_OBJ_DESC_SIZE, _("at least %d of the beasts %s been exterminated."), kills, plural(kills, _("has"), _("have")));
            memoryPrint(desc);
        }
    } else if (kills != 0u) {
        (void) snprintf(desc, MORIA_OBJ_DESC_SIZE, _("At least %d of these creatures %s"), kills, plural(kills, _("has"), _("have")));
        memoryPrint(desc);
        memoryPrint(_(" been killed by contributors to your monster memory."));
    } else {
        memoryPrint(_("No known battles to the death are recalled."));
    }
}
...
static bool memoryDepthFoundAt(uint8_t level, uint16_t kills) {
    ...
    if (level == 0) {
        known = true;
        memoryPrint(_(" It lives in the town"));
    } else if (kills != 0u) {
        known = true;
        ...
        obj_desc_t desc = {'\0'};
        (void) snprintf(desc, MORIA_OBJ_DESC_SIZE, _(" It is normally found at depths of %d feet"), level * 50);
        memoryPrint(desc);
    }
...
static bool memoryMovement(uint32_t rc_move, int monster_speed, bool is_known) {
    ...
    if ((rc_move & config::monsters::move::CM_ALL_MV_FLAGS) != 0u) {
        ...
        if ((rc_move & config::monsters::move::CM_RANDOM_MOVE) != 0u) {
            memoryPrint(_(recall_description_how_much[(rc_move & config::monsters::move::CM_RANDOM_MOVE) >> 3]));
            memoryPrint(_(" erratically"));
        }

        if (monster_speed == 1) {
            memoryPrint(_(" at normal speed"));
        } else {
            if ((rc_move & config::monsters::move::CM_RANDOM_MOVE) != 0u) {
                memoryPrint(_(", and"));
            }
            ...
            if (monster_speed <= 0) {
                if (monster_speed == -1) {
                    ...
                    memoryPrint(_(" incredibly"));
                }
                memoryPrint(_(" slowly"));
            } else {
                if (monster_speed == 3) {
                    memoryPrint(_(" very"));
                } else if (monster_speed > 3) {
                    memoryPrint(_(" unbelievably"));
                }
                memoryPrint(_(" quickly"));
            }
        }
    }
    ...
    if ((rc_move & config::monsters::move::CM_ATTACK_ONLY) != 0u) {
        ...
        memoryPrint(_(" does not deign to chase intruders"));
    }
    ...
    if ((rc_move & config::monsters::move::CM_ONLY_MAGIC) != 0u) {
        ...
        memoryPrint(_(" always moves and attacks by using magic"));
    }
...
static void memoryKillPoints(uint16_t creature_defense, uint16_t monster_exp, uint8_t level) {
    ...
    obj_desc_t desc = {'\0'};
    (void) snprintf(desc, MORIA_OBJ_DESC_SIZE, _(" creature is worth %d.%02d point%c"), quotient, remainder, plural);
    memoryPrint(desc);
    ...
    (void) snprintf(desc, MORIA_OBJ_DESC_SIZE, _(" for a%s %d%s level character."), q, py.misc.level, p);
    memoryPrint(desc);
}
...
static void memoryMagicSkills(uint32_t memory_spell_flags, uint32_t monster_spell_flags, uint32_t creature_spell_flags) {
        ...
        if ((spell_flags & (config::monsters::spells::CS_BR_LIGHT << i)) != 0u) {
            ...
            if (known) {
                if ((monster_spell_flags & config::monsters::spells::CS_FREQ) != 0u) {
                    memoryPrint(_(" It can breathe "));
                } else {
                    memoryPrint(_(" It is resistant to "));
                }
                known = false;
            ...
            memoryPrint(_(recall_description_breath[i]));
        }
    }
        ...
        if ((spell_flags & (config::monsters::spells::CS_TEL_SHORT << i)) != 0u) {
            ...
            if (known) {
                if ((memory_spell_flags & config::monsters::spells::CS_BREATHE) != 0u) {
                    memoryPrint(_(", and is also"));
                } else {
                    memoryPrint(_(" It is"));
                }
                memoryPrint(_(" magical, casting spells which "));
                known = false;
            ...
            memoryPrint(_(recall_description_spell[i]));
        }
    }
    ...
    if ((memory_spell_flags & (config::monsters::spells::CS_BREATHE | config::monsters::spells::CS_SPELLS)) != 0u) {
        ...
        if ((monster_spell_flags & config::monsters::spells::CS_FREQ) > 5) {
            obj_desc_t temp = {'\0'};
            (void) snprintf(temp, MORIA_OBJ_DESC_SIZE, _("; 1 time in %d"), creature_spell_flags & config::monsters::spells::CS_FREQ);
            memoryPrint(temp);
        }
        memoryPrint(_("."));
    }
...
static void memoryKillDifficulty(Creature_t const &creature, uint32_t monster_kills) {
    ...
    (void) snprintf(description, MORIA_OBJ_DESC_SIZE, _(" It has an armor rating of %d"), creature.ac);
    memoryPrint(description);

    (void) snprintf(description, MORIA_OBJ_DESC_SIZE,                                                         //
                    _(" and a%s life rating of %dd%d."),                                                      //
                    ((creature.defenses & config::monsters::defense::CD_MAX_HP) != 0 ? _(" maximized") : ""), //
...
static void memorySpecialAbilities(uint32_t move) {
    ...
    for (int i = 0; (move & config::monsters::move::CM_SPECIAL) != 0u; i++) {
        if ((move & (config::monsters::move::CM_INVISIBLE << i)) != 0u) {
            ...
            memoryPrint(_(recall_description_move[i]));
        }
    }
...
static void memoryWeaknesses(uint32_t defense) {
    ...
    for (int i = 0; (defense & config::monsters::defense::CD_WEAKNESS) != 0u; i++) {
        if ((defense & (config::monsters::defense::CD_FROST << i)) != 0u) {
            ...
            if (known) {
                memoryPrint(_(" It is susceptible to "));
                known = false;
            ...
            memoryPrint(_(recall_description_weakness[i]));
        }
    }
...
static void memoryAwareness(Creature_t const &creature, Recall_t const &memory) {
    if (memory.wake * memory.wake > creature.sleep_counter || memory.ignore == UCHAR_MAX || (creature.sleep_counter == 0 && memory.kills >= 10)) {
        ...
        if (creature.sleep_counter > 200) {
            memoryPrint(_("prefers to ignore"));
        } else if (creature.sleep_counter > 95) {
            memoryPrint(_("pays very little attention to"));
        } else if (creature.sleep_counter > 75) {
            memoryPrint(_("pays little attention to"));
        } else if (creature.sleep_counter > 45) {
            memoryPrint(_("tends to overlook"));
        } else if (creature.sleep_counter > 25) {
            memoryPrint(_("takes quite a while to see"));
        } else if (creature.sleep_counter > 10) {
            memoryPrint(_("takes a while to see"));
        } else if (creature.sleep_counter > 5) {
            memoryPrint(_("is fairly observant of"));
        } else if (creature.sleep_counter > 3) {
            memoryPrint(_("is observant of"));
        } else if (creature.sleep_counter > 1) {
            memoryPrint(_("is very observant of"));
        } else if (creature.sleep_counter != 0) {
            memoryPrint(_("is vigilant for"));
        } else {
            memoryPrint(_("is ever vigilant for"));
        }

        obj_desc_t text = {'\0'};
        (void) snprintf(text, MORIA_OBJ_DESC_SIZE, _(" intruders, which it may notice from %d feet."), 10 * creature.area_affect_radius);
        memoryPrint(text);
    }
}
...
static void memoryLootCarried(uint32_t creature_move, uint32_t memory_move) {
    ...
    if (carrying_chance == 1) {
        ...
        memoryPrint(_(" one or two"));
    } else {
        obj_desc_t msg = {'\0'};
        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _(" up to %d"), carrying_chance);
        memoryPrint(msg);
    }
    ...
    if ((memory_move & config::monsters::move::CM_CARRY_OBJ) != 0u) {
        ...
        memoryPrint(_(" treasures."));
    } else {
        memoryPrint(_(" treasure."));
    }
}
...
static void memoryAttackNumberAndDamage(Recall_t const &memory, Creature_t const &creature) {
    ...
    for (int i = 0; i < MON_MAX_ATTACKS; i++) {
        ...
        memoryPrint(_(recall_description_attack_method[attack_description_id]));
        ...
        if (attack_type != 1 || (dice.dice > 0 && dice.sides > 0)) {
            ...
            memoryPrint(_(recall_description_attack_type[attack_type]));
    ...
    if (attack_count != 0) {
        ...
        memoryPrint(_(" It has no physical attacks."));
    } else {
        memoryPrint(_(" Nothing is known about its attack."));
    }
}
...
int memoryRecall(int monster_id) {
    ...
    obj_desc_t msg = {'\0'};
    (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("The %s:\n"), _(creature.name));
    memoryPrint(msg);
    ...
    if ((defense & config::monsters::defense::CD_INFRA) != 0) {
        memoryPrint(_(" It is warm blooded"));
    }
    ...
    if ((defense & config::monsters::defense::CD_NO_SLEEP) != 0) {
        ...
        memoryPrint(_(" cannot be charmed or slept"));
    }
    ...
    if ((creature.movement & config::monsters::move::CM_WIN) != 0u) {
        memoryPrint(_(" Killing one of these wins the game!"));
    }

    memoryPrint(_("\n"));
    putStringClearToEOL(_("--pause--"), Coord_t{roff_print_line, 0});
...
void recallMonsterAttributes(char command) {
    ...
    for (int i = MON_MAX_CREATURES - 1; i >= 0; i--) {
        if (creatures_list[i].sprite == command && memoryMonsterKnown(creature_recall[i])) {
            if (n == 0) {
                int confirmed = getInputConfirmationWithAbort(40, _("You recall those details?"));
                if (confirmed != 1) {
                    break;
                }
