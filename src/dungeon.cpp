...
void dungeonDisplayMap() {
    ...
    putString(_("Hit any key to continue"), Coord_t{23, 23});
    ...
    for (uint8_t y = 0; y < MAX_HEIGHT; y++) {
        ...
        if (row != line) {
            if (line >= 0) {
                snprintf(line_buffer, MORIA_MESSAGE_SIZE, "|%s|", map);
...
void dungeonPlaceGold(Coord_t const &coord) {
    ...
    if (dg.floor[coord.y][coord.x].creature_id == 1) {
        printMessage(_("You feel something roll beneath your feet."));
    }
}
...
void dungeonPlaceRandomObjectAt(Coord_t const &coord, bool must_be_small) {
    ...
    if (dg.floor[coord.y][coord.x].creature_id == 1) {
        printMessage(_("You feel something roll beneath your feet.")); // -CJS-
    }
}
