...
static void trapOpenPit(Inventory_t const &item, int dam) {
    printMessage(_("You fell into a pit!"));

    if (py.flags.free_fall) {
        printMessage(_("You gently float down."));
        return;
    }
...
static void trapArrow(Inventory_t const &item, int dam) {
    if (playerTestBeingHit(125, 0, 0, py.misc.ac + py.misc.magical_ac, CLASS_MISC_HIT)) {
        ...
        printMessage(_("An arrow hits you."));
        return;
    }

    printMessage(_("An arrow barely misses you."));
}

static void trapCoveredPit(Inventory_t const &item, int dam, Coord_t coord) {
    printMessage(_("You fell into a covered pit."));
...
static void trapDoor(Inventory_t const &item, int dam) {
    ...
    printMessage(_("You fell through a trap door!"));
...
static void trapSleepingGas() {
    ...
    printMessage(_("A strange white mist surrounds you!"));
    ...
    printMessage(_("You fall asleep."));
}
...
static void trapHiddenObject(Coord_t coord) {
    ...
    printMessage(_("Hmmm, there was something under this rock."));
}
...
static void trapStrengthDart(Inventory_t const &item, int dam) {
    if (playerTestBeingHit(125, 0, 0, py.misc.ac + py.misc.magical_ac, CLASS_MISC_HIT)) {
        if (!py.flags.sustain_str) {
            ...
            printMessage(_("A small dart weakens you!"));
        } else {
            printMessage(_("A small dart hits you."));
        }
    } else {
        printMessage(_("A small dart barely misses you."));
    }
}
...
static void trapTeleport(Coord_t coord) {
    ...
    printMessage(_("You hit a teleport trap!"));
...
static void trapRockfall(Coord_t coord, int dam) {
    playerTakesHit(dam, _("a falling rock"));

    (void) dungeonDeleteObject(coord);
    dungeonPlaceRubble(coord);

    printMessage(_("You are hit by falling rock."));
}

static void trapCorrodeGas() {
    printMessage(_("A strange red gas surrounds you."));

    damageCorrodingGas(_("corrosion gas"));
}
...
static void trapFire(int dam) {
    ...
    damageFire(dam, _("a fire trap"));
}

static void trapAcid(int dam) {
    printMessage(_("You are splashed with acid!"));

    damageAcid(dam, _("an acid trap"));
}

static void trapPoisonGas(int dam) {
    printMessage(_("A pungent green gas surrounds you!"));

    damagePoisonedGas(dam, _("a poison gas trap"));
}

static void trapBlindGas() {
    printMessage(_("A black gas surrounds you!"));

    py.flags.blind += randomNumber(50) + 50;
}

static void trapConfuseGas() {
    printMessage(_("A gas of scintillating colors surrounds you!"));
...
static void trapConstitutionDart(Inventory_t const &item, int dam) {
    if (playerTestBeingHit(125, 0, 0, py.misc.ac + py.misc.magical_ac, CLASS_MISC_HIT)) {
        if (!py.flags.sustain_con) {
            ...
            printMessage(_("A small dart saps your health!"));
        } else {
            printMessage(_("A small dart hits you."));
        }
    } else {
...
static void playerStepsOnTrap(Coord_t coord) {
    ...
    switch ((TrapTypes) item.sub_category_id) {
        ...
        default:
            ...
            printMessage(_("Unknown trap value."));
            break;
    }
}
...
static void carry(Coord_t coord, bool pickup) {
    ...
    if (tile_flags == TV_GOLD) {
        ...
        itemDescription(description, item, true);
        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("You have found %d gold pieces worth of %s"), item.cost, description);
    ...
    if (inventoryCanCarryItemCount(item)) {
        ...
        if (pickup && config::options::prompt_to_pickup) {
            ...
            pickup = getInputConfirmation(_("Pick up ") + std::string(description));
        }
        ...
        if (pickup && !inventoryCanCarryItem(item)) {
            ...
            // change the period to a question mark
            replaceLastCharSafely(description, '?');
            pickup = getInputConfirmation(_("Exceed your weight limit to pick up ") + std::string(description));
        }
        ...
        if (pickup) {
            ...
            (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("You have %s (%c)"), description, locn + 'a');
            printMessage(msg);
            (void) dungeonDeleteObject(coord);
        }
    } else {
        itemDescription(description, item, true);
        (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("You can't carry %s"), description);
        printMessage(msg);
    }
}
    ...
    if (tile.creature_id < 2 || (!monster.lit && tile.feature_id >= MIN_CLOSED_SPACE)) {
        ...
        if (tile.feature_id <= MAX_OPEN_SPACE) {
            ...
            if ((py.running_tracker == 0) && tile.treasure_id != 0) {
                if (game.treasure.list[tile.treasure_id].category_id == TV_RUBBLE) {
                    printMessage(_("There is rubble blocking your way."));
                } else if (game.treasure.list[tile.treasure_id].category_id == TV_CLOSED_DOOR) {
                    printMessage(_("There is a closed door blocking your way."));
                }
            } else {
                playerEndRunning();
            }
