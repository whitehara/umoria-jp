...
void playerRestOn() {
    ...
    if (game.command_count > 0) {
        ...
        const char *rest_prompt = _("Rest for how long? ");
        putStringClearToEOL(rest_prompt, Coord_t{0, 0});

        if (getStringInput(rest_str, Coord_t{0, displayWidth(rest_prompt)}, 5)) {
            if (rest_str[0] == '*') {
                rest_num = -SHRT_MAX;
            } else {
    ...
    if (rest_num == -SHRT_MAX || (rest_num > 0 && rest_num <= SHRT_MAX)) {
        ...
        putStringClearToEOL(_("Press any key to stop resting..."), Coord_t{0, 0});
        putQIO();
    ...
    if (rest_num != 0) {
        printMessage(_("Invalid rest count."));
    }
    messageLineClear();
...
void playerDiedFromString(vtype_t *description, const char *monster_name, uint32_t move) {
    if (lang::currentLanguage() == "ja") {
        ...
        (void) snprintf(*description, MORIA_MESSAGE_SIZE, "%s", monster_name);
    } else if ((move & config::monsters::move::CM_WIN) != 0u) {
        (void) snprintf(*description, MORIA_MESSAGE_SIZE, _("The %s"), monster_name);
    } else if (isVowel(monster_name[0])) {
        (void) snprintf(*description, MORIA_MESSAGE_SIZE, _("an %s"), monster_name);
    } else {
        (void) snprintf(*description, MORIA_MESSAGE_SIZE, _("a %s"), monster_name);
    }
}
    ...
    for (spot.y = coord.y - 1; spot.y <= coord.y + 1; spot.y++) {
        for (spot.x = coord.x - 1; spot.x <= coord.x + 1; spot.x++) {
            ...
            if (item.category_id == TV_INVIS_TRAP) {
                ...
                obj_desc_t msg = {'\0'};
                (void) snprintf(msg, MORIA_OBJ_DESC_SIZE, _("You have found %s"), description);
                printMessage(msg);
                ...
                printMessage(_("You have found a secret door."));
                ...
                if ((item.flags & config::treasure::chests::CH_TRAPPED) > 1) {
                    if (!spellItemIdentified(item)) {
                        ...
                        printMessage(_("You have discovered a trap on the chest!"));
                    } else {
                        printMessage(_("The chest is trapped!"));
                    }
                }
            }
        }
...
void playerStrength() {
    ...
    if (item.category_id != TV_NOTHING && py.stats.used[PlayerAttr::A_STR] * 15 < item.weight) {
        if (!py.weapon_is_heavy) {
            printMessage(_("You have trouble wielding such a heavy weapon."));
        ...
        if (item.category_id != TV_NOTHING) {
            printMessage(_("You are strong enough to wield your weapon."));
        }
        playerRecalculateBonuses();
    }
    ...
    if (py.pack.heaviness != limit) {
        if (py.pack.heaviness < limit) {
            printMessage(_("Your pack is so heavy that it slows you down."));
        } else {
            printMessage(_("You move more easily under the weight of your pack."));
        }
...
void playerGainSpells() {
    ...
    if (new_spells == 0) {
        vtype_t tmp_str = {'\0'};
        (void) snprintf(tmp_str, MORIA_MESSAGE_SIZE, _("You can't learn any new %s!"), (stat == PlayerAttr::A_INT ? _("spells") : _("prayers")));
    ...
    if (new_spells > spell_id) {
        printMessage(_("You seem to be missing a book."));
    ...
    if (new_spells == 0) {
        ...
        char query;
        while ((new_spells != 0) && getMenuItemId(_("Learn which spell?"), query)) {
            int c = query - 'a';
        ...
        while (new_spells != 0) {
            ...
            vtype_t tmp_str = {'\0'};
            (void) snprintf(tmp_str, MORIA_MESSAGE_SIZE, _("You have learned the prayer of %s."), _(spell_names[spell_bank[id] + offset]));
            printMessage(tmp_str);
...
int playerWeaponCriticalBlow(int weapon_weight, int plus_to_hit, int damage, int attack_type_id) {
    ...
    if (randomNumber(5000) <= weapon_weight + 5 * plus_to_hit + (class_level_adj[py.misc.class_id][attack_type_id] * py.misc.level)) {
        ...
        if (weapon_weight < 400) {
            critical = 2 * damage + 5;
            printMessage(_("It was a good hit! (x2 damage)"));
        } else if (weapon_weight < 700) {
            critical = 3 * damage + 10;
            printMessage(_("It was an excellent hit! (x3 damage)"));
        } else if (weapon_weight < 900) {
            critical = 4 * damage + 15;
            printMessage(_("It was a superb hit! (x4 damage)"));
        } else {
            critical = 5 * damage + 20;
            printMessage(_("It was a *GREAT* hit! (x5 damage)"));
        }
    }
...
static void playerAttackMonster(Coord_t coord) {
    ...
    for (int i = blows; i > 0; i--) {
        if (!playerTestBeingHit(base_to_hit, (int) py.misc.level, total_to_hit, (int) creature.ac, PlayerClassLevelAdj::BTH)) {
            (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("You miss %s."), name);
            printMessage(msg);
            continue;
        }

        (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("You hit %s."), name);
        printMessage(msg);
        ...
        if (py.flags.confuse_monster) {
            ...
            if (((creature.defenses & config::monsters::defense::CD_NO_SLEEP) != 0) || randomNumber(MON_MAX_LEVELS) < creature.level) {
                (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("%s is unaffected."), name);
            } else {
                (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("%s appears confused."), name);
        ...
        if (monsterTakeHit(creature_id, damage) >= 0) {
            (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("You have slain %s."), name);
            printMessage(msg);
...
static void openClosedDoor(Coord_t coord) {
    ...
    if (item.misc_use > 0) {
        ...
        if (py.flags.confused > 0) {
            printMessage(_("You are too confused to pick the lock."));
        } else if (playerLockPickingSkill() - item.misc_use > randomNumber(100)) {
            printMessage(_("You have picked the lock."));
            py.misc.exp++;
            displayCharacterExperience();
            item.misc_use = 0;
        } else {
            printMessageNoCommandInterrupt(_("You failed to pick the lock."));
        }
    } else if (item.misc_use < 0) {
        // It's stuck

        printMessage(_("It appears to be stuck."));
    }
...
void playerOpenClosedObject() {
    ...
    if (no_object) {
        game.player_free_turn = true;
        printMessage(_("I do not see anything you can open there."));
    }
}
...
void playerCloseDoor() {
        ...
        if (item.category_id == TV_OPEN_DOOR) {
            if (tile.creature_id == 0) {
                if (item.misc_use == 0) {
                    ...
                    printMessage(_("The door appears to be broken."));
                }
            } else {
                objectBlockedByMonster(tile.creature_id);
            }
    ...
    if (no_object) {
        game.player_free_turn = true;
        printMessage(_("I do not see anything you can close there."));
    }
}
...
void playerAttackPosition(Coord_t coord) {
    ...
    if (py.flags.afraid > 0) {
        printMessage(_("You are too afraid!"));
        return;
    }
    ...
    for (int i = 31; mask != 0u; mask >>= 1, i--) {
        if ((mask & py.flags.spells_learnt) != 0u) {
            if (msp_ptr[i].level_required > py.misc.level) {
                ...
                vtype_t msg = {'\0'};
                (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("You have forgotten the %s of %s."), p, _(spell_names[i + offset]));
                printMessage(msg);
            } else {
                break;
            }
    ...
    for (int n = 0; ((py.flags.spells_forgotten != 0u) && (new_spells != 0) && (n < allowed_spells) && (n < 32)); n++) {
        ...
        if ((mask & py.flags.spells_forgotten) != 0u) {
            if (msp_ptr[order_id].level_required <= py.misc.level) {
                ...
                vtype_t msg = {'\0'};
                (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("You have remembered the %s of %s."), p, _(spell_names[order_id + offset]));
                printMessage(msg);
            } else {
                allowed_spells++;
            }
...
static void forgetSpells(int new_spells, const char *p, int offset) {
    ...
    for (int i = 31; (new_spells != 0) && (py.flags.spells_learnt != 0u); i--) {
        ...
        if ((mask & py.flags.spells_learnt) != 0u) {
            ...
            vtype_t msg = {'\0'};
            (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("You have forgotten the %s of %s."), p, _(spell_names[order_id + offset]));
            printMessage(msg);
        }
    }
}
...
void playerCalculateAllowedSpellsCount(int stat) {
    ...
    if (stat == PlayerAttr::A_INT) {
        magic_type_str = _("spell");
        offset = config::spells::NAME_OFFSET_SPELLS;
    } else {
        magic_type_str = _("prayer");
        offset = config::spells::NAME_OFFSET_PRAYERS;
    }
    ...
    if (new_spells != py.flags.new_spells_to_learn) {
        if (new_spells > 0 && py.flags.new_spells_to_learn == 0) {
            vtype_t msg = {'\0'};
            (void) snprintf(msg, MORIA_MESSAGE_SIZE, _("You can learn some new %s now."), (stat == PlayerAttr::A_INT) ? _("spells") : _("prayers"));
            printMessage(msg);
        }
...
char *playerRankTitle() {
    ...
    if (py.misc.level < 1) {
        ...
        p = _(class_rank_titles[py.misc.class_id][py.misc.level - 1]);
