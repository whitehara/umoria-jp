...
void putString(const char *out_str, Coord_t coord) {
    ...
    // Truncate by display column, not by byte count, so a multi-byte
    // (e.g. UTF-8) character is never cut in the middle.
    std::string str = truncateToWidth(out_str, 79 - coord.x);

    if (mvaddstr(coord.y, coord.x, str.c_str()) == ERR) {
        abort();
    }
}
...
// Tracks the display-column width of whatever is actually on screen on
// the message line (row 0) right now. This is deliberately NOT the same
// thing as displayWidth(messages[last_message_id]): that array entry is
// truncated to fit the fixed-size (byte-budgeted) vtype_t history buffer,
// which for multi-byte (e.g. Japanese) text can be narrower than what was
// actually drawn to the screen (a 3-bytes-per-character message hits the
// 79-byte buffer limit at a much smaller display width than the 79-column
// screen budget allows). Using the truncated stored width here would
// under-count the real on-screen content, causing the next message's
// "-more-" indicator or combined text to be placed too far left —
// overlapping and clobbering the tail of what's still visible.
static int message_line_display_width = 0;
...
void messageLinePrintMessage(std::string message) {
    ...
    // truncate message if it's too long! (by display column, not byte count,
    // so a multi-byte, e.g. UTF-8, character is never cut in the middle.)
    std::string truncated_message = truncateToWidth(message, 79);

    addstr(truncated_message.c_str());
    message_line_display_width = displayWidth(truncated_message);
...
void printMessage(const char *msg) {
    ...
    if (message_ready_to_print) {
        // Use the actual on-screen display width (see
        // message_line_display_width's comment), not
        // displayWidth(messages[last_message_id]) — that stored copy can be
        // narrower than what's really on screen once byte-budget truncation
        // kicks in on multi-byte (e.g. Japanese) text.
        old_len = message_line_display_width + 1;
        ...
        if ((msg == nullptr) || new_len + old_len + 2 >= 73) {
            ...
            putString(_(" -more-"), Coord_t{MSG_LINE, old_len});
    ...
    if (combine_messages) {
        ...
        message_line_display_width = old_len + 2 + new_len;
        // Defensive only: combine_messages is only set when
        // new_len + old_len + 2 < 73, so this can't actually exceed the
        // 79-column screen width. Clamped anyway in case that invariant
        // ever changes.
        if (message_line_display_width > 79) {
            message_line_display_width = 79;
        }

        // Combine by budgeting raw bytes (not display columns) so a
        // multi-byte (e.g. UTF-8) character is never split across the
        // fixed-size vtype_t buffer boundary.
        std::string combined = std::string(messages[last_message_id]) + "  " + msg;
        std::string truncated = truncateToByteCapacity(combined, MORIA_MESSAGE_SIZE - 1);
        (void) strncpy(messages[last_message_id], truncated.c_str(), MORIA_MESSAGE_SIZE);
        ...
        std::string truncated = truncateToByteCapacity(msg, MORIA_MESSAGE_SIZE - 1);
        (void) strncpy(messages[last_message_id], truncated.c_str(), MORIA_MESSAGE_SIZE);
        messages[last_message_id][MORIA_MESSAGE_SIZE - 1] = '\0';
    }
}
    ...
    while (true) {
        ...
        if (ch == EOF) {
            ...
            if (eof_flag > 100) {
                ...
                (void) strcpy(game.character_died_from, _("(end of input: panic saved)"));
                if (!saveGame()) {
                    (void) strcpy(game.character_died_from, _("panic: unexpected eof"));
...
void waitForContinueKey(int line_number) {
    putStringClearToEOL(_("[ press any key to continue ]"), Coord_t{line_number, 23});
    (void) getKeyInput();
