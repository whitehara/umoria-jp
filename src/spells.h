...
// `spell_name` is looked up in the translation catalog internally (via
// `_()`), so callers may pass either the raw English name (e.g.
// `spell_names[8]`, "Lightning Ball") or an already-translated string
// (e.g. `_("Lightning Ball")`, or a composed phrase like
// `death_description`). Passing an already-translated string is safe: a
// second catalog lookup on a string that isn't itself a known msgid is a
// harmless pass-through. Prefer passing the raw English name where
// practical, so `scripts/extract_strings.py` can find and catalog it.
