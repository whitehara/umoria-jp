...
## umoria-jp (Japanese Localization fork)

* Added a Japanese message catalog (`data/lang/ja/`) covering UI messages,
  monster/item names, race/class labels, class rank titles, and
  unidentified-item flavor words.
* Added a minimal `.po`-subset i18n core (`src/lang.cpp`/`lang.h`) with
  `_()`/`C_()` catalog lookups, `-l <lang>` / `MORIA_LANG` language
  selection, and automatic fallback to English for missing translations.
* Added display-width-aware string helpers (`src/helpers.h`:
  `displayWidth()`, `truncateToWidth()`, `truncateToByteCapacity()`,
  `padToDisplayWidth()`) and fixed a number of hardcoded-byte-width UI
  layout bugs (cursor placement, fixed-width label fields, status bar
  clearing) that only manifested once multi-byte translated text was
  displayed.
* Switched the ncurses dependency to the wide-character build
  (`ncursesw`), required for correct multi-byte character rendering.
* See [`docs/TRANSLATING.md`](docs/TRANSLATING.md) for how the
  localization mechanism works.
